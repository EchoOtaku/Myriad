...
pub(crate) async fn start_unified_server(config: AppConfig) -> anyhow::Result<()> {
    ...
    let api_router = api_router
        ...
        // their own DefaultBodyLimit from `federation::limits` (inbox 96 MiB,
        // authenticated 112 MiB) which **may exceed** this outer layer — see
        // `federation::limits` tests. Do not assume 50 MiB caps public inbox.
