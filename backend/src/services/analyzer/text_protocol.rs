...
pub(crate) fn response_text(provider: AiProvider, body: &Value) -> Result<String> {
    if body.get("error").is_some_and(|error| !error.is_null()) {
        anyhow::bail!("AI provider returned an error");
    }
    ...
    if text.trim().is_empty() {
        anyhow::bail!("AI provider returned no text content");
    }
...
mod tests {
    ...
    #[test]
    fn combines_text_blocks_without_exposing_thinking() {
        assert_eq!(response_text(AiProvider::Anthropic, &json!({"content":[
            {"type":"thinking", "text":"private"}, {"type":"text", "text":"first"}, {"type":"text", "text":" second"}
        ]})).unwrap(), "first second");
        assert_eq!(
            response_text(
                AiProvider::OpenAIResponses,
                &json!({"error":null,"output":[
    ...
    fn protocols_have_distinct_endpoints_and_shapes() {
        ...
        assert_eq!(
            endpoint(
                AiProvider::Anthropic,
                Some("https://claude.example/v1/messages")
            ),
    ...
    fn protocols_extract_blocking_and_streaming_text() {
        assert_eq!(
            response_text(
                AiProvider::OpenAIResponses,
                &json!({"error":null,"output":[{"content":[{"type":"output_text","text":"done"}]}]})
            )
            .unwrap(),
            "done"
        );
