...
impl DeliveryCoordinator {
    pub async fn claim(&self, intent: &SpeakIntent) -> Option<DeliveryClaim> {
        let lock = {
            ...
            users.retain(|_, slot| Arc::strong_count(&slot.lock) > 1 || slot.used.elapsed() < IDLE_RETENTION);
            if !users.contains_key(&intent.user_id) && users.len() >= MAX_USERS { return None; }
            let slot = users.entry(intent.user_id).or_insert_with(|| UserSlot {
                lock: Arc::new(AsyncMutex::new(History::default())), used: Instant::now(),
            });
        ...
        if intent.expires_at <= now || history.topics.contains_key(&intent.topic)
...
impl DeliveryClaim {
    pub fn delivered(&mut self, intent: &SpeakIntent, repeat_minutes: i64) {
        ...
        self.history.sources.insert(intent.source_event_id.clone(), until.max(intent.expires_at));
    }
}
...
mod tests {
    ...
    fn intent(user: i32, source: &str) -> SpeakIntent {
        new_speak_intent(user, source.into(), "agent.merope.greeting".into(), "hello".into(), Default::default(), None)
    }
