...
impl PlatformFetcher {
    ...
    /// 获取 X 用户最近一年的推文数据
    #[allow(dead_code)]
    pub async fn fetch_twitter_tweets(
        &self,
        user_id: &str,
        bearer_token: &str,
    ) -> Result<Vec<serde_json::Value>> {
    ...
    /// 获取 X 用户统计数据（包含一年内推文）
    #[allow(dead_code)]
    pub async fn fetch_twitter_stats(
        &self,
        username: &str,
        bearer_token: &str,
    ) -> Result<serde_json::Value> {
