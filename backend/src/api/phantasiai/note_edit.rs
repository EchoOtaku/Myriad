...
use myriad_phantasi_notes::{ai_edit::{SYSTEM_PROMPT, build_prompt, validate_result}, render_markdown_preview};

fn failure(status: StatusCode, code: &str) -> Response {
    (status, Json(json!({"success":false,"code":code,"error":code}))).into_response()
}

pub(super) async fn edit_note(State(db): State<DatabaseConnection>, headers: HeaderMap, Json(request): Json<Value>) -> Response {
    if let Err(response) = super::verify_admin(&headers, &db).await { return response; }
    let prompt = match build_prompt(&request) {
        Ok(prompt) => prompt,
        Err(code) => return failure(StatusCode::BAD_REQUEST, code),
    };
    let Some(analyzer) = super::create_ai_analyzer_for_tier(crate::config::ModelTier::Standard).await else {
    ...
    let call = crate::services::ai_cost_ledger::with_site_ai_ledger(owner, "phantasiai", "note_edit",
    ...
    let response = match tokio::time::timeout(std::time::Duration::from_secs(240), call).await {
        ...
        Ok(Err(error)) => {
            ...
            let context_limit = ["context_length", "context window", "context length", "too many tokens", "token limit", "maximum context", "input too long", "request too large"]
                .iter().any(|part| message.contains(part));
            return failure(StatusCode::BAD_GATEWAY, if context_limit { "note_ai_context_limit" } else { "note_ai_failed" });
        },
    ...
    match validate_result(&request, &response) {
        Ok(content_md) => {
            ...
            (StatusCode::OK, Json(json!({"content_md":content_md,"html":html}))).into_response()
        },
