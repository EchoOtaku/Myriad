...
use crate::models::topics::{select_random_topics, Topic};
...
lazy_static::lazy_static! {
    static ref REPORT_CACHE: Arc<Mutex<HashMap<String, (PersonalReport, DateTime<Utc>)>>> = 
...
pub async fn fetch_all_data(
    ...
    if let Ok(github_username) = std::env::var("GITHUB_USERNAME") {
        ...
        match fetcher.fetch_github_user(&github_username, github_token.as_deref()).await {
            Ok(user_data) => {
...
pub async fn generate_report(
    ...
    tracing::info!("Selected topics: {:?}", selected_topics.iter().map(|t| &t.title).collect::<Vec<_>>());
...
用轻松、有趣、有洞察力的语言。让人读完说"哇，真的懂我"。只返回JSON，不要其他文字。"#,
    ...
    tracing::info!("✓ Personal report generated successfully with {} cards", report.cards.len());
...
fn get_card_style(category: &str) -> (String, String) {
    match category {
        ...
        "宇宙玄学" => ("from-indigo-400 to-purple-400".to_string(), "🌌".to_string()),
        "占卜预测" => ("from-violet-400 to-fuchsia-400".to_string(), "🔮".to_string()),
        "社交关系" => ("from-emerald-400 to-green-400".to_string(), "🎪".to_string()),
