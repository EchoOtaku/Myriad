...
async fn get_platform_data(platform: &str) -> Result<SmartFilteredData, String> {
    ...
    // 全局处理锁，防止并发重复处理
    static PROCESSING_LOCK: Lazy<Arc<Mutex<()>>> = Lazy::new(|| Arc::new(Mutex::new(())));

    let smart_cache_path = PathBuf::from("./cache/smart_filtered_data.json");
    let raw_cache_path = PathBuf::from("./cache/platform_data.json");

    // 1. 尝试直接读取智能缓存
    if smart_cache_path.exists() {
        if let Ok(content) = fs::read_to_string(&smart_cache_path) {
            if let Ok(data) = serde_json::from_str::<Value>(&content) {
                if let Some(platform_data) = data.get(platform) {
                    if let Ok(smart_data) =
                        serde_json::from_value::<SmartFilteredData>(platform_data.clone())
                    {
                        tracing::debug!("✓ Loaded {} from smart cache", platform);
                        return Ok(smart_data);
                    }
                }
            }
        }
    ...
    // 2. 如果智能缓存不存在或没有该平台数据，使用锁保护重新生成
    if raw_cache_path.exists() {
        tracing::info!(
            "Smart cache miss for {}, attempting to generate from raw data...",
            platform
        );

        // 获取锁，防止并发重复处理
        let _lock = PROCESSING_LOCK.lock().await;
                ...
                if let Ok(data) = serde_json::from_str::<Value>(&content) {
                    if let Some(platform_data) = data.get(platform) {
                        if let Ok(smart_data) =
                            serde_json::from_value::<SmartFilteredData>(platform_data.clone())
                        {
                            tracing::info!("✓ {} cache generated by another request", platform);
                            return Ok(smart_data);
                        }
                    }
                }
        ...
        // 真正需要处理时才读取和处理
        tracing::warn!("⚠️  Processing raw data for all platforms (this may take time)...");
        ...
        let data_to_process = if let Some(d) = raw_json.get("data") {
            d
        } else {
            &raw_json
        };

        // 调用 SmartFilter 处理并保存
        if let Err(e) = SmartFilter::process_and_save_all(data_to_process) {
            tracing::error!("❌ Failed to process smart filter: {}", e);
            return Err(format!("Failed to process raw data: {}", e));
        }

        tracing::info!("✓ Smart cache regenerated successfully");
    ...
    Err(format!("No data found for platform: {}", platform))
}
