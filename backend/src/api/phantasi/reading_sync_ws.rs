//! Phantasi reading state, item detail, stats, and WebSocket.
use axum::{
    Extension, Json,
    extract::{Path, State},
    http::StatusCode,
...
use sea_orm::{
    ...
    QueryTrait, Statement, TransactionTrait, sea_query::Expr,
};
use serde_json::json;
...
use super::helpers::{
    phantasi_http_err, phantasi_store_http, get_user_and_admin_status, get_user_id_from_headers,
};
...
pub(crate) async fn get_item(
    ...
    // 获取文章
    let item = phantasi_items::Entity::find_by_id(id).one(&db).await;
    ...
    match item {
        Ok(Some(item)) => {
            ...
            if let Ok(Some(source)) = source {
                ...
                let (is_read, is_starred, read_progress, state_revision) = if let Some(uid) =
                    user_id
                {
                    let state = phantasi_user_states::Entity::find()
        ...
        Err(e) => Err(phantasi_store_http("find article", e)),
    }
}
...
fn visible_state_sources(is_admin: bool) -> sea_orm::Select<phantasi_sources::Entity> {
                ...
                Ok(saved) => {
                    ...
                    if let Some(read) = is_read {
                        if read != was_read {
                            ...
                            .await
                            .map_err(|error| phantasi_store_http("update unread count", error))?;
                        }
                    }
                    transaction
                        .commit()
...
pub(crate) async fn update_source_unread_count<C: ConnectionTrait>(
    ...
    let stmt = Statement::from_sql_and_values(
        ...
        "UPDATE phantasi_sources SET unread_count = GREATEST(0, unread_count + $1) WHERE id = $2",
...
pub(crate) async fn mark_all_read(
    ...
    let mut source_filter = phantasi_sources::Entity::find();
    if !is_admin {
        source_filter = source_filter.filter(phantasi_sources::Column::AdminOnly.eq(false));
    }

    if let Some(source_id) = req.source_id {
        source_filter = source_filter.filter(phantasi_sources::Column::Id.eq(source_id));
    }
    if let Some(ref category) = req.category {
        source_filter = source_filter.filter(phantasi_sources::Column::Category.eq(category));
    }

    let sources: Vec<phantasi_sources::Model> = source_filter
        .all(&db)
        .await
    ...
    query = query.filter(phantasi_items::Column::SourceId.is_in(source_ids.clone()));

    if let Some(before) = req.before {
        let dt = chrono::DateTime::from_timestamp_millis(before)
            .ok_or_else(|| phantasi_http_err(StatusCode::BAD_REQUEST, "Invalid reading cutoff"))?;
        query = query.filter(phantasi_items::Column::PublishedAt.lt(dt));
    }
    ...
    let items: Vec<(i32, i32)> = query
        ...
        .all(&transaction)
        .await
        .map_err(|error| phantasi_store_http("load bulk articles", error))?;
    ...
    // 批量查询所有已存在的状态
    let existing_states: Vec<phantasi_user_states::Model> = phantasi_user_states::Entity::find()
        .filter(phantasi_user_states::Column::UserId.eq(user_id))
        .filter(phantasi_user_states::Column::ItemId.is_in(item_ids.clone()))
        .order_by_asc(phantasi_user_states::Column::ItemId)
        .lock_exclusive()
        .all(&transaction)
        .await
        .map_err(|error| phantasi_store_http("load bulk reading states", error))?;
    ...
    if !unread_state_ids.is_empty() {
        let update_result = phantasi_user_states::Entity::update_many()
            .col_expr(phantasi_user_states::Column::IsRead, Expr::value(true))
            .col_expr(phantasi_user_states::Column::ReadAt, Expr::value(now))
            .col_expr(phantasi_user_states::Column::UpdatedAt, Expr::value(now))
            .filter(phantasi_user_states::Column::Id.is_in(unread_state_ids))
            .exec(&transaction)
            .await
            .map_err(|error| phantasi_store_http("update bulk reading states", error))?;
    ...
    if !missing_item_ids.is_empty() {
        let new_states: Vec<phantasi_user_states::ActiveModel> = missing_item_ids
            .iter()
            .map(|&item_id| phantasi_user_states::ActiveModel {
                user_id: Set(user_id),
                item_id: Set(item_id),
                is_read: Set(true),
        ...
        phantasi_user_states::Entity::insert_many(new_states)
            .exec(&transaction)
            .await
            .map_err(|error| phantasi_store_http("insert bulk reading states", error))?;
        {
            marked += insert_count;
            // 统计每个 source 被标记的数量
    ...
    for (source_id, count) in source_marked_counts {
        update_source_unread_count(&transaction, source_id, -count)
            .await
            .map_err(|error| phantasi_store_http("update bulk unread counts", error))?;
    }
...
async fn apply_synced_state(
    ...
    let source_id = match phantasi_items::Entity::find_by_id(state_item.item_id)
        .filter(
            phantasi_items::Column::SourceId.in_subquery(visible_state_sources(is_admin).into_query()),
        )
        .select_only()
        .column(phantasi_items::Column::SourceId)
        .lock_exclusive()
        ...
        let (read_count, starred) = if !all_item_ids.is_empty() {
            tokio::join!(
                ...
                phantasi_user_states::Entity::find()
                    .filter(phantasi_user_states::Column::UserId.eq(uid))
                    .filter(phantasi_user_states::Column::ItemId.is_in(all_item_ids.clone()))
            ...
            // 空列表时仍需查询收藏数
            let starred = phantasi_user_states::Entity::find()
                .filter(phantasi_user_states::Column::UserId.eq(uid))
                .filter(phantasi_user_states::Column::IsStarred.eq(true))
                .count(&db)
                .await;
...
mod sync_transaction_tests {
    ...
    async fn count_failure_rolls_back_state_and_success_commits_both() {
        ...
        failed
            .execute_unprepared("UPDATE phantasi_atomic_probe SET revision = revision + 1 WHERE id = 1")
            .await
            .unwrap();
