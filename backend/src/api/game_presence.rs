...
fn read_cache(key: &str) -> Option<GamePresenceData> {
...
fn store_cache(key: &str, data: &GamePresenceData) {
...
pub async fn get_game_presence(
    ...
    // 基础校验，防滥用
    if account_id.len() > 64 || !account_id.chars().all(|c| {
    ...
    let result = match platform.as_str() {
        "hoyolab" | "hoyoverse" | "miyoushe" | "enka" => {
            fetch_enka(&account_id, &game).await
        }
...
fn parse_enka_response(
    ...
    let level = player
        .get("level")
        .and_then(|v| v.as_i64())
        .or_else(|| player.get("level").and_then(|v| v.as_u64()).map(|u| u as i64));
    ...
    let achievements = player
        ...
        .or_else(|| player.get("recordInfo").and_then(|r| r.get("achievementCount")))
        .and_then(|v| v.as_i64());
...
async fn fetch_xbox(gamertag: &str) -> Result<GamePresenceData, String> {
    ...
    let person = search
        ...
        .and_then(|a| a.first())
        .cloned()
        .or_else(|| search.get("profileUsers").and_then(|v| v.as_array()).and_then(|a| a.first()).cloned())
...
async fn fetch_psn(online_id: &str) -> Result<GamePresenceData, String> {
    ...
    // NPSSO → access token (mobile client credentials used by community tools)
    ...
    .await
    {
        if let Some(p) = legacy.get("profile") {
            ...
            if let Some(pres) = p.get("presences").and_then(|v| v.as_array()).and_then(|a| a.first()) {
        ...
        .await
        {
            if let Some(domain) = search
                .get("domains")
                .and_then(|v| v.as_array())
                .and_then(|a| a.iter().find(|d| d.get("domain").and_then(|x| x.as_str()) == Some("SocialAllAccounts")))
            {
        ...
        if let Some(aid) = &account_id {
            // Trophy summary v2
            let trophy_url = format!(
                "https://m.np.playstation.com/api/trophy/v1/users/{aid}/trophySummary"
            );
