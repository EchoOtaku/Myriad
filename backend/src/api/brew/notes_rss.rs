...
async fn load_notes_rss_channel(db: &DatabaseConnection) -> Result<NotesRssChannel, String> {
    ...
    let channel_link = public_absolute_url(base.as_deref(), "/brew");
...
mod tests {
    ...
    fn sample_channel() -> NotesRssChannel {
        NotesRssChannel {
            ...
            link: "https://ex.com/brew".into(),
            description: "Hello <world>".into(),
            language: "zh-CN".into(),
            feed_url: "https://ex.com/brew/notes.xml".into(),
            last_build: "Mon, 14 Sep 2026 00:00:00 +0000".into(),
            items: vec![NotesRssItem {
                guid: "note:abc".into(),
                title: "A & B".into(),
                link: "https://ex.com/brew/item/12".into(),
    ...
    fn empty_channel_is_valid_rss() {
        let xml = render_notes_rss(&NotesRssChannel {
            ...
            language: "en".into(),
            feed_url: "/brew/notes.xml".into(),
            last_build: "Mon, 14 Sep 2026 00:00:00 +0000".into(),
            items: Vec::new(),
        });
        assert!(xml.contains(r#"<rss version="2.0""#));
        assert!(xml.contains("<atom:link href=\"/brew/notes.xml\""));
    ...
    fn render_escapes_and_keeps_html_in_content() {
        ...
        assert!(!xml.contains("brew_note_docs"));
    }

    #[test]
    fn brew_parser_reads_the_notes_feed() {
        let xml = render_notes_rss(&sample_channel());
