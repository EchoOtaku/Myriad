    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .create_table(
                Table::create()
                    ...
                    .col(ColumnDef::new(Configurations::Value).json_binary().not_null())
                    .col(ColumnDef::new(Configurations::Description).text())
                    .col(ColumnDef::new(Configurations::IsPublic).boolean().default(false))
                    .col(
        ...
        let insert = Query::insert()
            ...
            .values_panic([
                "ai_provider".into(),
                "\"openai\"".into(),
                "AI service provider".into(),
                true.into(),
            ])
            .values_panic([
                "ai_model".into(),
                "\"gpt-4\"".into(),
                "AI model to use for analysis".into(),
                true.into(),
            ])
            .to_owned();
