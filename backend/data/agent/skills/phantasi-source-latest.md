---
name: 某订阅源最新文章
description: 按订阅源名称 phantasi.sources 宽松匹配定位 sourceId，再 phantasi.items 取最新文章；本地查询，禁止纯列表联网
category: phantasi
triggers:
  - 订阅源
  - 这个源
...
用户指定一个订阅源名称（参数 `${source_name}`），先在本地 `phantasi.sources` 宽松匹配，再列出该源最新文章。

# 硬性约束

1. **必须先 sources 再 items**：先从 `phantasi.sources` 拿到真实整数 `id`，再调用 `phantasi.items` 的 **`sourceId`**。
2. **禁止**在「本地有该源 / 有匹配文章」时使用 `ai.webSearch`。默认路径全程本地。
3. 禁止编造 `sourceId`；禁止把源名字符串塞进 `sourceId`。
4. `phantasi.items` **优先传 `sourceId`**（来自 sources 步骤）；不要只靠 `sourceName` 当唯一路径。
...
调用 `phantasi.items`，**必须**传步骤 1 的整数 id：
...
- 可选：`phantasi.page` 且 `level: "items"` + 同一 `sourceId`。
