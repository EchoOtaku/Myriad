# Digital Life 3D pipeline

## Goal

Digital Life may use a fully 3D character, but the product target is not the
...
All management routes are under `/api/digital-life/3d`:
...
does not remove already persisted Digital Life assets.
...
- Renderer integration: the frontend client contract is ready, but the preview
  branch does not yet contain the independent Digital Life rig/viewer worktree.
