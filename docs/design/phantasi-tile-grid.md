...
- Demo：[phantasi-tile-redesign.canvas.tsx](/Users/hitomi/.cursor/projects/Users-hitomi-GitHub-Myriad/canvases/phantasi-tile-redesign.canvas.tsx)
- 阶段看板：[phantasi-tile-implementation.canvas.tsx](/Users/hitomi/.cursor/projects/Users-hitomi-GitHub-Myriad/canvases/phantasi-tile-implementation.canvas.tsx)
...
5. 阶段 3 先上首页，再动 `PhantasiSourceGrid`。不要一上来改 946 行网格。

---

## 1. 目标

把 Phantasi 订阅源页从「固定三列 CSS Grid + 手工 `card_size`」换成首页同款的**虚拟坐标磁贴墙**：
...
- `PhantasiSourceGrid` 的布局引擎、磁贴形态、排序、横向分页
- 磁贴符合 `WidgetComponentProps`，注册 `phantasi-source` / `phantasi-topic` / `phantasi-featured`
- 文章主题聚类（关键词先上，AI 后换）
- 四个游客态既有缺陷
- 后端：`pulses`（派生数组）+ `phantasi_items.topic` 列

### 这次不碰

- `PhantasiReader`（全屏阅读器）
- `PhantasiFeedList` / `ItemCard`（文章列表，下一独立任务）
...
| 把 Phantasi 的 FLIP 搬进首页 | 首页继续用 `WidgetGrid` 自己的 transition |
...
### 前端 `frontend/src/types/phantasi.ts`
...
// PhantasiItem / PhantasiItemPreview 新增
topic?: string | null
// 预定义主题 key（如 "engineering"），不是展示文案。
// null / 缺失的文章不参与聚类。

// PhantasiItemsQuery 新增
topic?: string
```

`PhantasiTopic` **不是**后端类型，由 `clusterTopics()` 在前端派生。
...
| `phantasi_items` | `topic TEXT NULL` | 关键词或 AI 写入 |
| `ItemsQuery` / `list_items` | `topic: Option<String>` | 与现有 `category` 同级过滤 |

**不要新建 numbered migration。** 本仓库的政策是：

1. 把列写进 `backend/migrations/003_phantasi_system.rs`（greenfield）
2. 把列写进 `backend/src/db/schema_check/tables_phantasi.rs` 的 `phantasi_items` `TableDef`
3. 写进 `backend/src/models/entities/phantasi_items.rs` 的 `Model` + `ItemResponse`
...
函数：`tileLayout(source, role) → PhantasiTileLayout`
...
```ts
export type PhantasiViewerRole = 'guest' | 'member' | 'admin'

export interface ScoreFactors {
  own: number     // 0 | 1   isOwnPhantasiSource
...
export const SCORE_WEIGHTS: Record<PhantasiViewerRole, ScoreFactors> = {
...
export function phantasiScore(s: PhantasiSource, role: PhantasiViewerRole, now: number): number {
...
类型现在是 `'update' | 'custom' | 'category' | 'random' | 'pinyin'`，在 `components/phantasi/types.ts` 与 `manager/modes/types.ts` **两处**都要加 `'smart' | 'topic'`。
...
```ts
export type PhantasiCard =
  | { kind: 'source'; key: string; src: PhantasiSource; size: WidgetSize }
  | { kind: 'topic'; key: string; topic: PhantasiTopic; size: WidgetSize }

export type PackedCard = PhantasiCard & { x: number; y: number }

export function packPhantasiCards(
  cards: PhantasiCard[],
  cols: number,
  rows: number,
  opts?: { breakOn?: (prev: PhantasiCard, next: PhantasiCard) => boolean },
): PackedCard[][]
```
...
分页：外层 `width: ${pages * 100}%`，`transform: translateX(-${page * 100 / pages}%)`。页码进 `sessionStorage`，切排序时重置为 0。游客也要有圆点指示器（控制面板没有，Phantasi 必须有）。
...
3. `Phantasi.tsx` 二级导航「收藏」对游客隐藏。现在能点，但 `viewMode === 'starred' && isAuthenticated` 让主区空白。
...
// phantasi-topic
{ topicKey: string }
// phantasi-featured：不绑源，内部跑 smart 取 top source + top topic
```
...
```ts
'phantasi-source':    { defaultSize: '4x2', supportedSizes: ['2x2','4x2','4x4'], component: ... }
'phantasi-topic':     { defaultSize: '4x2', supportedSizes: ['2x2','4x2','4x4'], component: ... }
'phantasi-featured':  { defaultSize: '4x4', supportedSizes: ['4x2','4x4'],       component: ... }
```

同步改：`BUILTIN_WIDGET_ORDER`、`WIDGET_NAME_KEY`、`i18n/en-US.json`（及 zh-CN / ja-JP）的 `widgets`。id 是 kebab，i18n key 是 camelCase（`phantasiSource` / `phantasiTopic` / `phantasiFeatured`）。`getWidgetTranslationKey` 已做这个转换，但 `WIDGET_NAME_KEY` 仍要显式写。

---

## 13. 类型契约

新建 `frontend/src/components/phantasi/logic/`：

### `score.ts`

见 §8。导出 `scoreFactors`、`phantasiScore`、`SCORE_WEIGHTS`、`SCORE_BUCKET = 0.05`。
...
export function tileLayout(s: PhantasiSource, role: PhantasiViewerRole): PhantasiTileLayout
...
export function clusterTopics(items: PhantasiItem[], now: number): PhantasiTopic[]
export function inferTopicByKeywords(item: PhantasiItem): string | null
```
...
| 路径 | 操作 | 做什么 |
| --- | --- | --- |
| `components/phantasi/logic/score.ts` | 新建 | 评分 |
| `components/phantasi/logic/layout.ts` | 新建 | 构图 + 尺寸 |
| `components/phantasi/logic/pack.ts` | 新建 | 装箱 |
| `components/phantasi/logic/topics.ts` | 新建 | 聚类 + 关键词 |
| `components/phantasi/logic/*.test.ts` | 新建 | 单测 |
| `components/phantasi/tileGridFlag.ts` | 新建 | `localStorage['phantasi_tile_grid'] === '1'` |
| `components/phantasi/tiles/TileShell.tsx` | 新建 | 薄封装 WidgetShell |
| `components/phantasi/tiles/TileCover.tsx` | 新建 | 封面，无图返回 null |
| `components/phantasi/tiles/Cadence.tsx` | 新建 | 节律图 |
| `components/phantasi/tiles/MinorRow.tsx` | 新建 | 次条 |
| `components/phantasi/tiles/PhantasiSourceTile.tsx` | 新建 | 五种构图 × 三档尺寸 |
| `components/phantasi/tiles/PhantasiTopicTile.tsx` | 新建 | 主题卡 |
| `components/phantasi/tiles/PhantasiFeaturedTile.tsx` | 新建 | 首页综合卡 |
| `components/phantasi/tiles/rotation.css` | 新建 | keyframes |
| `components/phantasi/PhantasiPageDots.tsx` | 新建 | 分页点 |
| `components/phantasi/manager/modes/TopicFeedMode.tsx` | 新建 | 控制岛 |
| `views/PhantasiTilePreview.tsx` | 新建 | `import.meta.env.DEV` 预览，24 种组合 |
| `components/phantasi/PhantasiSourceGrid.tsx` | 大改 | 虚拟坐标 + 分页 + flag 切旧网格 |
| `components/phantasi/types.ts` | 改 | SortMode |
| `components/phantasi/manager/modes/types.ts` | 改 | SortMode + ControlMode |
| `components/phantasi/manager/modes/DefaultMode.tsx` | 改 | 下拉加 smart / topic |
| `components/phantasi/manager/ControlIsland.tsx` | 改 | 游客 tip；topic-feed |
| `components/phantasi/cards/SourceCard.tsx` | 改 | 游客未读点（PR 1）；PR 9 删除 |
| `views/Phantasi.tsx` | 改 | 收藏 Tab、markRead、topic-feed |
...
| `db/schema_check/tables_phantasi.rs` | 改 | TableDef |
| `migrations/003_phantasi_system.rs` | 改 | greenfield 列 |
| 关键词打标 | 新建小模块 | 入库同步 |
| Phantasiai 批量打标 | 后做 | 每小时，只处理 30 天内 `topic IS NULL` |
...
- `packPhantasiCards` 不重叠、不丢卡、`breakOn` 换页
...
- `GET /api/phantasi/sources` 每源带 `pulses`，长度 ≤ 60，值为距今天数
- 上千篇文章的源被截断，响应没有明显变慢（一次窗口查询，禁止 N+1）
- `GET /api/phantasi/items?topic=engineering` 只返回该主题
- `topic IS NULL` 的文章不进过滤结果
- 入库后关键词能写出 topic；AI 失败保持 NULL

### PR 4 验收

- `/dev/phantasi-tiles`（仅 DEV）铺开：6 构图 × 3 尺寸 × 4 空数据
...
4. MinorRow
5. PhantasiSourceTile · icon
6. feature
7. list + 轮播
8. numeric
9. cadence 构图
10. PhantasiTopicTile
...
- 建议索引：`CREATE INDEX IF NOT EXISTS idx_phantasi_items_topic ON phantasi_items (topic) WHERE topic IS NOT NULL`
- 写进 `schema_check/expected_indexes.rs`

### 打标

1. **入库同步关键词**（`inferTopicByKeywords` 的 Rust 版或共享词表）。成本接近零。
2. **每小时 AI 批**：近 30 天、`topic IS NULL`、每批 20–50 篇。复用 Phantasiai 调用链。
...
框架：`node:test` + `tsx`。在 `frontend/` 跑 `pnpm test:unit -- src/components/phantasi/logic`。

没有 Testing Library，没有视觉回归，没有 E2E。组件靠 `/dev/phantasi-tiles` 人工扫。

后端：inline `#[cfg(test)]`。Phantasi 已有 DB-gated smoke（`PHANTASI_TEST_DATABASE_URL`），pulses SQL 加进去。
...
| 现有 | 处置 | 原因 |
| --- | --- | --- |
| `usePhantasiCardStagger` | 保留 | 只看 index |
...
| `PhantasiSourceGrid` resize | ~120 | `handleResizeStart`、拉伸条 |
| `PhantasiSourceGrid` CSS Grid 容器 | ~80 | `grid-cols-1/2/3`、`gridAutoRows` |
| i18n `feedTypeRss/Atom/Json` | 9 键 × 3 | 只被 SourceCard 用；`feedTypeLink` / `feedTypePhantasiaiDesc` 可能 EditModal 还在用，逐键 grep |
| `tileGridFlag` | ~15 | 最容易忘，变成永久债 |

保留：`CardSize` 与 `phantasi_sources.card_size`（改语义为锁定）、`sort_order`、`getIconUrl` / `getImageUrl`、`getPlainText`、`normalizeThemeColor`、`PHANTASI_MINE_CATEGORY` / `isOwnPhantasiSource`、整个 FeedList/ItemCard。
...
```ts
// frontend/src/components/phantasi/tileGridFlag.ts
const KEY = 'phantasi_tile_grid'

export function isPhantasiTileGridEnabled(): boolean {
  try {
...
打开：控制台 `localStorage.setItem('phantasi_tile_grid','1')` 后刷新。`PhantasiSourceGrid` 顶部分支。PR 9 删除模块与分支。
...
<Route path="/dev/phantasi-tiles" element={<SuspensePage><PhantasiTilePreview /></SuspensePage>} />
```
...
| Phantasi 用 4 行还是 8 行 | **4 行**，与首页一致。Demo 16×8 只是画布预览 |
...
- 重写 `PhantasiReader` 或 `ItemCard`
- 把坐标写入数据库
- 新建 numbered migration（改 `003` + `tables_phantasi.rs`）
- 引入 Redux/Zustand
- 把主题名写死在组件里（走 i18n）
- 在组件里写第二套窄屏判断
