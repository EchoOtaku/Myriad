...
| --- | --- | --- | --- |
| frontend dev server | `1102` | `pnpm dev` in `frontend/` | Serves the app; Astro dev proxy forwards the same public path set as production (API + federation public paths). Dev proxy does **not** upgrade WebSockets and does **not** split persona/federation onto workers. |
