...
| frontend | `4321` | none | Internal Docker network only; reached through `proxy`. |
| backend | `3000` | none | Internal Docker network only; reached through `proxy` for `/api/*` and `/health`. |
| postgres | `5432` | none | Internal Docker network only; data lives in `./pgdata`. |
| updater | `9090` | none | Internal Docker network only; browser normally reaches it through backend `/api/admin/updater/*`. |
...
| --- | --- | --- | --- |
| frontend dev server | `4321` | `pnpm dev` in `frontend/` | Serves the app and proxies `/api/*` to backend. |
| backend | `3000` | `cargo run --bin myriad-backend` in `backend/` | Direct API development endpoint. |
...
| updater harness | `9090` | `./scripts/dev/dev.sh start updater` or `start all-updater` | Optional. Exposes updater to the host backend at `http://127.0.0.1:9090`. |
