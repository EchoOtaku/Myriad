...
| [examples/v3.env.example](./examples/v3.env.example) | 按 kiseki.blog 运维形状脱敏的 `.env` 示例（`v0.3.7` 统一 tag / preview / `HTTP_PORT=8080`） |
...
3. **编辑 `.env`（保留旧 secret，只加缺口）**：
   ...
   示例中的 `MYRIAD_TAG` / `PROXY_TAG` / `UPDATER_TAG=v0.3.7`、`CHANNEL=preview`、
