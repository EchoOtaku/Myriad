...
async function getGeoFromFallbackServices(): Promise<GeoLocationData | null> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.latitude && data.longitude) {
        const lat = typeof data.latitude === 'string' ? parseFloat(data.latitude as unknown as string) : data.latitude;
        const lon = typeof data.longitude === 'string' ? parseFloat(data.longitude as unknown as string) : data.longitude;
