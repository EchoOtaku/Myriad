...
const TRACKING_PARAMS = [
  ...
  'fbclid', 'gclid', 'ref', 'source', 'share', 'from', 'app', 'isappinstalled',
  'wfr', 's_r', 'nsukey', 'scene', 'sub_channel', 'key', 'tn', 'timestamp',
  'sign', 'token', '_t', 't', 'random', 'r', '_',
  // 微信
  'chksm', 'mpshare', 'isappinstalled', 'from_msgid', 'from_itemidx',
  // 微博
  'weibo_id', 'mb_id', 'is_hot', 'hottop_id',
  // 知乎
...
const DANGEROUS_TAGS = [
  ...
  'select', 'textarea', 'option', 'optgroup', 'datalist',
];
...
const DANGEROUS_ATTRS = [
  ...
  'ondrag', 'ondragstart', 'ondragend', 'ondrop', 'onscroll', 'onwheel',
  'ontouchstart', 'ontouchmove', 'ontouchend', 'onpointerdown', 'onpointerup',
  'formaction', 'xlink:href', 'xmlns', 'srcdoc',
];
...
function processVideos(html: string): string {
  // 处理 <video> 标签
  let result = html.replace(/<video([^>]*)>/gi, (match, attrs) => {
    if (attrs.includes('class=')) {
...
function processRssHubSpecific(html: string): string {
  ...
  // 处理类别标签
  result = result.replace(/<category>([^<]*)<\/category>/gi, 
...
function processSourceSpecific(html: string, options: ProcessOptions): string {
  ...
  // 移除一些网站的广告占位
  result = result.replace(/<div[^>]*class="[^"]*ad[s]?[-_]?[^"]*"[^>]*>[\s\S]*?<\/div>/gi, '');
  result = result.replace(/<aside[^>]*class="[^"]*ad[s]?[-_]?[^"]*"[^>]*>[\s\S]*?<\/aside>/gi, '');
