...
export class PodcastPlayer {
  ...
  static selectVoicePair(voices: SpeechSynthesisVoice[], lang: string): { voiceA: SpeechSynthesisVoice | null; voiceB: SpeechSynthesisVoice | null } {
    ...
    console.log('[PodcastPlayer] Available voices for language', lang, ':', 
    ...
    if (!femaleVoice) femaleVoice = filtered.find(v => v !== maleVoice) || filtered[0];
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      for (const annotation of sortedAnnotations) {
        ...
        if (annotation.type === 'reference' && referencePositions.has(annotation.term)) {
          ...
          const isOnlyOccurrence = fullText.indexOf(annotation.term, bestPos + 1) === -1 
            && fullText.lastIndexOf(annotation.term, bestPos - 1) === -1;
        ...
        const annotationId = String(rawId).replace(/[^a-zA-Z0-9_-]/g, '');
