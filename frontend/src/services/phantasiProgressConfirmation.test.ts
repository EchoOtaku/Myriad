...
import { markAllRead, markRead, syncReadingStates, updateReadProgress } from './phantasiApi'
...
it('requires the target article confirmation and rejects per-item failures', async () => {
  ...
  try {
    ...
    await assert.rejects(updateReadProgress(7, 70, { expectedRevision: 5 }), PhantasiSyncConflictError)
    assert.equal(sent?.states[0]?.expected_revision, 5)
    result = { synced: 0, conflicts: [{ item_id: 7, server_revision: 4 }] }
    await assert.rejects(updateReadProgress(7, 50), error => error instanceof PhantasiSyncConflictError && error.serverRevision === 4)
