import { useState, useEffect, useMemo, useCallback, memo, useRef, lazy, Suspense } from 'react';
...
const getBilibiliProxyUrl = (cover?: string, title?: string): string => {
  ...
  if (cover.includes('hdslb.com') || cover.includes('bilibili.com')) {
    return `${API_URL ? API_URL : ''}/api/proxy/image?url=${encodeURIComponent(cover)}`;
  }
  return cover;
};

// 迷你组件：B站弹幕云 (优化：使用 memo + 优化动画性能)
