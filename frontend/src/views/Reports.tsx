...
const SteamStatsWidget = memo(
  ({
    ...
    const score = useMemo(
      () => data?.hardcore_score || 0,
      [data?.hardcore_score],
    )
    const type = useMemo(
      () => data?.player_type || defaultPlayerType,
      [data?.player_type, defaultPlayerType],
    )
...
const GithubStatsWidget = memo(
  ({
    ...
    const level = useMemo(
      () => data?.contribution_level || defaultLevel,
      [data?.contribution_level, defaultLevel],
    )
    ...
    const getLevelColor = (level: string) => {
      if (
        level.includes(levelKeywords.legendary) ||
        level.includes(levelKeywords.core)
      ) {
        return '#22c55e'
      }
      if (
        level.includes(levelKeywords.senior) ||
        level.includes(levelKeywords.prolific)
      ) {
        return '#3b82f6'
      }
        ...
        if (Array.isArray(data.mood_keywords)) {
          if (data.mood_keywords.length > 0) {
            if (typeof data.mood_keywords[0] === 'string') {
              ...
              moodKeywords = data.mood_keywords.map(
                (tag: string, i: number) => ({
                  tag,
      ...
      <AnimatePresence mode="wait">
        ...
        libraryItems.length === 0 ? (
          ...
          >
            <MusicStatsWidget
              data={processedData}
              tenThousandSuffix={tenThousandSuffix}
              triggerKey={showOverview}
            />
          </motion.div>
        ) : (
    ...
    (platformId: string, content: any) => {
      setCardContents((prev) => {
        ...
        if (
          ...
          if (
            prevContent.id === content.id &&
            prevContent.timestamp === content.timestamp
          ) {
            return prev
          }
        }
              ...
              <div className="h-full glass rounded-2xl">
                ...
                <div className="h-full flex flex-col md:flex-row overflow-hidden rounded-2xl">
                  ...
                  <div className="w-full md:w-1/3 p-6 md:p-8 flex flex-col border-b md:border-b-0 md:border-r border-gray-100 dark:border-neutral-800 bg-white/50 dark:bg-neutral-950/50 backdrop-blur-xl overflow-y-auto">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4 ${translatedPlatforms.find((p) => p.id === selectedReport.platform)?.bg} ${translatedPlatforms.find((p) => p.id === selectedReport.platform)?.text}`}
                    >
                      {
                        translatedPlatforms.find(
                          (p) => p.id === selectedReport.platform,
                        )?.icon
                      }
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      >
                        <motion.div
                          animate={
                            loadingPlatform === selectedReport.platform
                              ? { rotate: 360 }
                              : {}
                          }
                          transition={{
                    ...
                    <div className="space-y-4 flex-1">
                      ...
                      <div className="p-3 rounded-xl bg-gray-50 dark:bg-neutral-900 border border-gray-100 dark:border-neutral-700 h-32">
                        <div className="w-full h-full">
                          {selectedReport.platform === 'bilibili' && (
                            <BilibiliWidget
                              data={selectedReport.card_visuals}
                              showOverview={showOverview}
                              defaultDanmaku={defaultDanmaku}
                            />
                          )}
                          {selectedReport.platform === 'steam' && (
                            <SteamWidget
                              data={selectedReport.card_visuals}
                              showOverview={showOverview}
                              defaultPlayerType={defaultPlayerType}
                          ...
                          {selectedReport.platform === 'github' && (
                            <GithubWidget
                              data={selectedReport.card_visuals}
                              showOverview={showOverview}
                          ...
                          {selectedReport.platform === 'netease' && (
                            <NeteaseWidget
                              data={selectedReport.card_visuals}
                              showOverview={showOverview}
                              tenThousandSuffix={
                                t.reportsPage.tenThousandSuffix
                              }
                            />
                          )}
                          {selectedReport.platform === 'bangumi' && (
                            <BangumiWidget
                              data={selectedReport.card_visuals}
                              showOverview={showOverview}
                            />
                  ...
                  <div className="flex-1 p-6 md:p-8 overflow-y-auto bg-white dark:bg-neutral-950">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                        {t.reportsPage.deepInsightReport}
                      </h3>
                    </div>

                    <div className="grid gap-4">
                      {selectedReport.insights.map(
                            ...
                            {platform.id === 'bilibili' ? (
                              <>
                                  ...
                                  {isLoading ? (
                                    ...
                                    <div className="w-full h-full">
                                      <BilibiliWidget
                                        ...
                                        onContentChange={
                                          contentChangeHandlers[platform.id]
                                        }
                                        showOverview={showOverview}
                                        defaultDanmaku={defaultDanmaku}
                                      />
                                ...
                                <motion.div
                                  ...
                                  initial={false}
                                  animate={{
                                    width: cardContents[platform.id]
                                      ? 'auto'
                                      : '32px',
                                  }}
                                  transition={{
                                  ...
                                  <div
                                    ...
                                    style={{
                                      background: cardContents[platform.id]
                                        ...
                                        : 'rgba(0, 161, 214, 0.15)',
                                      border:
                                        '1px solid rgba(0, 161, 214, 0.3)',
                                      padding: cardContents[platform.id]
                                        ? '0 8px 0 8px'
                                        : '0 8px',
                                    }}
                                  >
                                  ...
                                  <div
                                    ...
                                    style={{
                                      ...
                                      padding: cardContents[platform.id]
                                        ? '4px 8px'
                                        : '0 8px',
                                      height: cardContents[platform.id]
                                        ? 'auto'
                                        : '32px',
                                    }}
                                  >
                                    ...
                                    <AnimatePresence>
                                      {cardContents[platform.id] && (
                                        ...
                                        >
                                          {cardContents[platform.id].titles.map(
                                ...
                                >
                                  <div
                                    ...
                                    style={{
                                      ...
                                      border:
                                        '1px solid rgba(240, 145, 153, 0.3)',
                                      padding: cardContents[platform.id]
                                        ? '4px 8px'
                                        : '0 8px',
                                      height: cardContents[platform.id]
