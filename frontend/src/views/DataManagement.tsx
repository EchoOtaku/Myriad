...
export default function DataManagement() {
  ...
  const loadAllStatuses = async () => {
    ...
    try {
      ...
      const statuses: PlatformStatus[] = PLATFORMS.map((platform) => {
        ...
        const cacheInfo = cacheData?.caches?.find(
          (c: CacheInfo) => c.platform === platform.id,
        )
  ...
  // 刷新单个平台原始数据
  const refreshSinglePlatform = async (
    platformId: string,
    platformName: string,
  ) => {
    if (
      !confirm(
        t.dataManagement.confirmRefreshData.replace('{platform}', platformName),
      )
    ) {
      return
    }
  ...
  // 处理平台智能过滤
  const handleProcessPlatform = async (
    platformId: string,
    platformName: string,
  ) => {
  ...
  const handleClearCache = async (platformId: string, platformName: string) => {
    if (
      !confirm(
        t.dataManagement.confirmClearCache.replace('{platform}', platformName),
      )
    ) {
      return
    }
      ...
      <div className="modern-config-container">
        ...
        <div className="card-header" style={{ marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>
            {t.dataManagement.dataManagementTitle}
          </h2>
          ...
          {PLATFORMS.map((platform, index) => {
            ...
            return (
              <motion.div
                ...
                animate={
                  isPageReady ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }
                }
                transition={{
                  delay: isPageReady ? 0.1 + index * 0.08 : 0,
                  duration: 0.3,
                }}
              >
                {/* 平台标题 */}
                  ...
                  <div className="section-header-left">
                    ...
                    <div>
                      <h3
                        ...
                        className="dark:text-gray-100"
                      >
                        {(platform as any).nameKey
                          ? t.dataManagement[
                              (platform as any)
                                .nameKey as keyof typeof t.dataManagement
                            ]
                          : platform.name}
                      </h3>
                    </div>
                ...
                >
                  ...
                  <div className="field-control">
                    <button
                      ...
                      disabled={
                        refreshingPlatform === platform.id || statusLoading
                      }
                ...
                <div className="config-field-row" style={{ marginBottom: 0 }}>
                  ...
                  >
                    <button
                      ...
                      disabled={
                        isProcessing || !status?.has_raw_data || statusLoading
                      }
                    ...
                    {cache && (
                      <button
                        ...
                        disabled={
                          clearingPlatform === platform.id || statusLoading
                        }
        ...
        >
          <p className="info-title" style={{ marginBottom: '0.75rem' }}>
            {t.dataManagement.usageTitle}
          </p>
          <div
            className="info-text"
          ...
          >
            <p style={{ marginBottom: '0.5rem' }}>
              {t.dataManagement.usageRawData}
            </p>
            <p style={{ marginBottom: '0.5rem' }}>
              {t.dataManagement.usageSmartFilter}
            </p>
            <p style={{ marginBottom: 0 }}>
              {t.dataManagement.usageBackground}
            </p>
          </div>
        </div>
      </div>
