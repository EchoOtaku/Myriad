...
export default function Account() {
  ...
  async function handleChangePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (newPassword.length < 8) {
      setPasswordError(t.accountPage.passwordMinLength);
      return;
    }

    if (newPassword !== confirmPassword) {
      setPasswordError(t.accountPage.passwordMismatch);
      return;
    }

    if (oldPassword === newPassword) {
      setPasswordError(t.accountPage.passwordSameAsOld);
      return;
    }
    ...
    try {
      ...
      if (!csrfToken) {
        setPasswordError(t.accountPage.csrfTokenError);
        setSubmitting(false);
        return;
      }
      ...
      if (response.ok && result.success) {
        alert(t.accountPage.passwordChangeSuccess);
        setPasswordError('');
        e.currentTarget.reset();
      } else {
        setPasswordError(result.message || result.error || t.accountPage.changeFailed);
      }
    } catch (error) {
      setPasswordError(t.accountPage.networkError);
    } finally {
      setSubmitting(false);
    }
  }
            ...
            <div className="flex items-start justify-between gap-4 md:gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3 md:mb-4">
                  ...
                  <div>
                    <h2 className="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-100">{t.accountPage.accountInfo}</h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t.accountPage.githubBinding}</p>
                  </div>
                </div>
          ...
          <div className="mb-4 md:mb-6">
            <div className="glass rounded-xl p-4 md:p-5">
              <div className="flex items-center gap-3 mb-3 md:mb-4">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-100">{t.accountPage.changePassword}</h2>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t.accountPage.changePasswordDesc}</p>
                </div>
              </div>
                ...
                <div className="space-y-3 md:space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      minLength={8}
                      className="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-neutral-600 dark:bg-neutral-800 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent min-h-[44px]"
                      placeholder={t.accountPage.atLeast8Chars}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      minLength={8}
                      className="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-neutral-600 dark:bg-neutral-800 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent min-h-[44px]"
                      placeholder={t.accountPage.enterNewPasswordAgain}
                    />
                  </div>
                  ...
                  >
                    {submitting ? t.accountPage.changing : t.accountPage.changePasswordBtn}
                  </button>
                </div>
              </form>
            </div>
