...
import { isPageVisible, onVisibility, batchWrite } from './animation/core';
...
function applyRippleDistortion(
  ...
  const rCx: number[] = [], rCy: number[] = [];
  const rWaveFrontScaled: number[] = [], rTimeDecay: number[] = [];
  const rElapsed: number[] = [], rMinRSq: number[] = [], rMaxRSq: number[] = [];
  
  let globalMinX = width, globalMaxX = 0;
  let globalMinY = height, globalMaxY = 0;
...
export function useEvocativeWallpaper(
  ...
  useEffect(() => {
    ...
    const rippleAnimationLoop = (now: number) => {
      if (!s.active || !s.rippleCanvas || !s.rippleCtx || !s.sourceImageData) return;
    ...
    const startRipple = async (x: number, y: number) => {
      ...
      if (s.activeRipples.length === 0 || !s.sourceImageData) {
        s.sourceImageData = await captureWallpaperToCanvas(
          s.el, s.rippleCanvas, s.rippleCtx, s.blurCurrentBlur, rippleScale
        );
        ...
        if (enableDynamicBlur && !s.blurIdle) {
          ...
          if (diffAbs < THRESHOLD * 0.6) {
            ...
            if ((currentAbs < 0 ? -currentAbs : currentAbs) > THRESHOLD * 0.6) {
              ...
              batchWrite(() => {
                if (s.el) s.el.style.filter = `${BLUR_PREFIX}${rounded}${BLUR_SUFFIX}`;
              });
            }
          } else {
