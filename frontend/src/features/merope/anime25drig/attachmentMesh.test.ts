...
test('samples the current final host geometry including later hair motion', () => {
  ...
  const mesh={rest,deformed:rest.slice(),indices:new Uint16Array([0,1,2])}
  ...
  for(let frame=1;frame<=3;frame++) {
    for(let i=0;i<rest.length;i+=2) {mesh.deformed[i]=rest[i]+frame*5;mesh.deformed[i+1]=rest[i+1]-frame*2}
    sampleAttachmentMesh(sample,point)
