...
export function bindAttachmentMesh(mesh: AttachmentMesh, x: number, y: number): AttachmentMeshSample | null {
  const r = mesh.rest
...
export function sampleAttachmentMesh(sample: AttachmentMeshSample, output: {x:number,y:number}): void {
  output.x=sample.x; output.y=sample.y
  for(let k=0;k<3;k++) {
    const i=sample.indices[k]*2, w=sample.weights[k]
