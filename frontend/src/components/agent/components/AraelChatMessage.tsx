...
function parseInline(text: string): React.ReactNode[] {
  ...
  for (let match = inlineRe.exec(text); match !== null; match = inlineRe.exec(text)) {
    if (match.index > lastIndex) {
      nodes.push(text.slice(lastIndex, match.index))
    }
    if (match[1]) {
      // 行内代码
      nodes.push(<code key={key++} className="arael-md-code">{match[1].slice(1, -1)}</code>)
    }
    else if (match[3]) {
      // 图片 ![alt](url)
      nodes.push(<img key={key++} className="arael-md-img" src={match[3]} alt={match[2] || ''} loading="lazy" />)
    }
    else if (match[4] && match[5]) {
      // 链接 [text](url)
      nodes.push(<a key={key++} className="arael-md-link" href={match[5]} target="_blank" rel="noopener noreferrer">{match[4]}</a>)
    }
    else if (match[6]) {
      // **加粗**
...
function renderMessageContent(message: string): React.ReactNode {
  ...
  while (i < lines.length) {
    ...
    if (headingMatch) {
      ...
      elements.push(<Tag key={key++} className="arael-md-heading">{parseInline(headingMatch[2])}</Tag>)
      i++
      continue
    }
    ...
    if (BLOCKQUOTE_RE.test(line)) {
      ...
      while (i < lines.length) {
        ...
        if (!qm)
          break
        quoteLines.push(<p key={key++} className="arael-md-p">{parseInline(qm[1])}</p>)
        i++
      }
      elements.push(<blockquote key={key++} className="arael-md-blockquote">{quoteLines}</blockquote>)
      continue
    }

    // 表格 | col | col |
    if (line.includes('|') && line.trim().startsWith('|') && i + 1 < lines.length && TABLE_SEP_RE.test(lines[i + 1].trim())) {
      ...
      elements.push(
        <table key={key++} className="arael-md-table">
          <thead><tr>{headers.map((h, ci) => <th key={ci}>{parseInline(h)}</th>)}</tr></thead>
          <tbody>
            {rows.map((row, ri) => (
    ...
    if (UL_RE.test(line)) {
      ...
      while (i < lines.length && UL_RE.test(lines[i])) {
        items.push(<li key={key++}>{parseInline(lines[i].replace(UL_RE, ''))}</li>)
        i++
      }
      elements.push(<ul key={key++} className="arael-md-list">{items}</ul>)
      continue
    }
    ...
    if (OL_RE.test(line)) {
      ...
      while (i < lines.length && OL_RE.test(lines[i])) {
        items.push(<li key={key++}>{parseInline(lines[i].replace(OL_RE, ''))}</li>)
        i++
      }
      elements.push(<ol key={key++} className="arael-md-list">{items}</ol>)
      continue
    }
    ...
    // 普通段落
    elements.push(<p key={key++} className="arael-md-p">{parseInline(line)}</p>)
    i++
  }
        ...
        <div className="arael-question-options">
          {pendingQuestion.options!.map((option, idx) => {
            ...
            return (
              ...
              >
                {isSelected && (
                  <svg className="arael-option-check" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export const AraelChatMessage: React.FC<AraelChatMessageProps> = React.memo(({
    ...
    return (
      <div className="arael-msg arael-msg-user">
        <div className="arael-msg-bubble">
          ...
          <div className="arael-msg-time">{formatTime(message.createdAt, locale)}</div>
        </div>
      </div>
    )
  }
  ...
  const finishedSteps = liveSteps.filter(s => s.status === 'completed' || s.status === 'error')
  ...
  const formatStepLabel = (step: ExecutionStep) => {
    ...
    if (idx && total && total > 1) {
      return t.arael.stepLabel.replace('{idx}', String(idx)).replace('{name}', step.name)
    }
    return step.name
  }
  ...
  const thinkingLabel = (() => {
    if (isProcessing) {
      if (runningStep)
        return formatStepLabel(runningStep)
      if (planDescs.length > 0 && liveSteps.length === 0)
        return planDescs[0]
      return null
    }
    if (isCompleted || isError) {
      ...
      if (lastStep)
        return formatStepLabel(lastStep)
      return null
    }
    return null
  })()
  ...
  const showThinkingHeader = !!exec && (isProcessing || isCompleted || isError)
      ...
      <div className="arael-msg-body">
        <div className="arael-msg-bubble">
          ...
          {showThinkingHeader && (
            ...
            >
              <span className={`arael-thinking-header-text ${isProcessing ? 'arael-current-step-text' : ''} ${isCompleted ? 'arael-thinking-header-text-done' : ''} ${isError ? 'arael-thinking-header-text-error' : ''}`}>
                {isProcessing
              ...
              <span className="arael-thinking-header-arrow">{thinkingExpanded ? '▾' : '▸'}</span>
            </div>
          )}
          ...
          {showThinkingHeader && thinkingExpanded && (
            <div className="arael-thinking-panel">
              ...
              <div className="arael-thinking-panel-timer">
                ...
                <span>
                  {isProcessing
                    ? t.arael.elapsedTime.replace('{time}', formatElapsed(elapsed))
                    : totalDurationMs > 0
                      ? t.arael.totalTime.replace('{time}', formatDuration(totalDurationMs))
                      : t.arael.completed}
                </span>
              </div>
              ...
              {(finishedSteps.length > 0 || runningStep) && (
                <div className="arael-thinking-panel-steps">
                  {finishedSteps.map((step) => {
                    ...
                    return (
                      <div key={step.id} className={`arael-thinking-panel-step arael-thinking-panel-step-${step.status}`}>
                        <span className="arael-thinking-panel-step-icon">
                          {step.status === 'completed' ? '✓' : '✗'}
                        </span>
                        <span className="arael-thinking-panel-step-name">{formatStepLabel(step)}</span>
                        {tier && (
                        ...
                        {step.durationMs != null && step.durationMs > 0 && (
                          <span className="arael-thinking-panel-step-dur">{formatDuration(step.durationMs)}</span>
                        )}
                        {step.degraded && (
                          <span className="arael-thinking-panel-step-note">{t.arael.autoDegraded}</span>
                        )}
                        {step.retryAttempt != null && step.retryAttempt > 0 && (
                          <span className="arael-thinking-panel-step-note">
                            {t.arael.retryCount.replace('{n}', String(step.retryAttempt))}
                          </span>
                        )}
                      </div>
                    )
              ...
              {planDescs.length > 0 && liveSteps.length === 0 && (
                <div className="arael-thinking-panel-steps">
                  {planDescs.map((desc, idx) => (
                    <div key={idx} className={`arael-thinking-panel-step ${idx === 0 ? 'arael-thinking-panel-step-running' : 'arael-thinking-panel-step-pending'}`}>
                      <span className={`arael-thinking-panel-step-name ${idx === 0 ? 'arael-current-step-text' : ''}`}>
                        {planDescs.length > 1 ? t.arael.stepLabel.replace('{idx}', String(idx + 1)).replace('{name}', desc) : desc}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="arael-thinking-panel-footer">
                  {(proCount > 0 || stdCount > 0 || exec?.skillName) && (
                    <div className="arael-thinking-panel-meta">
                      ...
                      {exec?.skillName && <span className="arael-thinking-panel-skill">{exec.skillName}</span>}
                    </div>
                  )}
                  {exec?.recalledMemories && exec.recalledMemories.length > 0 && (
                    <div className="arael-thinking-panel-memories">
                      {exec.recalledMemories.map((mem, i) => (
                        <div key={i} className="arael-thinking-panel-memory-item">{mem}</div>
                      ))}
                    </div>
                  )}
                </div>
          ...
          {!message.pendingQuestion && message.suggestions && message.suggestions.length > 0 && onSuggestionClick && (
            <div className="arael-question-input-area">
              <div className="arael-question-options">
