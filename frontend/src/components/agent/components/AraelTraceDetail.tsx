...
export const AraelTraceDetail: React.FC<AraelTraceDetailProps> = ({ task }) => {
  ...
  if (task.status !== 'completed' && task.status !== 'error') return null
  if (task.steps.length === 0 && !trace) return null

  const hasDetailData =
    trace || task.steps.some((s) => s.tierUsed || s.durationMs)
    ...
    <div className="arael-trace-detail">
      {(proCount > 0 || stdCount > 0) && (
        <div className="arael-trace-tiers">
          {proCount > 0 && (
            <span className="arael-tier-pro">Pro x{proCount}</span>
          )}
          {stdCount > 0 && (
            <span className="arael-tier-std">Std x{stdCount}</span>
          )}
        </div>
      )}
      ...
      <div className="arael-trace-content">
        {loading && (
          <div className="arael-trace-loading">{t.arael.loading}</div>
        )}
        ...
        {hasDetailData && (
          <div className="arael-trace-steps">
            {(trace?.steps ?? task.steps).map((step) => {
              const stepId = (
                'stepId' in step ? step.stepId : step.id
              ) as string
              const taskStep = task.steps.find((s) => s.id === stepId)
              const capId =
                'capabilityId' in step
                  ? (step as { capabilityId: string }).capabilityId
                  : undefined
              const label = getCategoryLabel(
                taskStep?.capabilityCategory ?? capId?.split('.')[0],
              )
              const tier =
                ('tierUsed' in step ? step.tierUsed : undefined) ??
                taskStep?.tierUsed
              ...
              const degraded =
                ('degraded' in step
                  ? (step as { degraded?: boolean }).degraded
                  : false) ?? taskStep?.degraded
              const retry = taskStep?.retryAttempt

              return (
                <div
                  key={stepId}
                  className={`arael-trace-step ${success ? '' : 'arael-trace-step-failed'}`}
                >
                  ...
                  <span className="arael-trace-step-meta">
                    ...
                    {duration != null && duration > 0 && (
                      <span className="arael-trace-step-duration">
                  ...
                  {!success &&
                    ...
                    (step as { error?: string }).error && (
                      <div className="arael-trace-step-error">
                        {(step as { error?: string }).error}
                      </div>
                    )}
                  {degraded && (
        ...
        {task.recalledMemories && task.recalledMemories.length > 0 && (
          <div className="arael-trace-memories">
            <span className="arael-trace-memories-label">
              {t.arael.recalledMemories}
            </span>
            {task.recalledMemories.map((mem, i) => (
              <div key={i} className="arael-trace-memory-item">
                {mem}
              </div>
            ))}
          </div>
