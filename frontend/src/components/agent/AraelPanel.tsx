...
import { speechToText, audioToBase64, getSpeechStatus } from '../../services/speechApi';
...
// 智能提示词生成
const getSmartGreeting = (pathname: string, historyCount: number): string => {
...
export const AraelPanel: React.FC = () => {
  ...
  const activeTasks = useMemo(() =>
    tasks.filter(t => t.status === 'processing' || t.status === 'waiting'),
    [tasks]
  );
  ...
  const smartGreeting = useMemo(() =>
    getSmartGreeting(location.pathname, presetHistory.length),
    [visibility, location.pathname] // visibility 变化时重新生成
  );
  ...
  const checkMovement = useCallback((e: MouseEvent | TouchEvent) => {
    if (!longPressStartRef.current || !isLongPressingRef.current) return;
                      ...
                      <span className="arael-tasks-title">
                        {activeTasks.length > 0 ? (
                          <>
                            ...
                            Arael 正在处理 {activeTasks.length} 个任务
                          </>
                        ) : (
                          <>{smartGreeting}</>
                        )}
