...
const PlatformIcon = memo(({ platformName }: { platformName: string }) => {
  ...
  const icon = useMemo(() => {
    switch (platformName.toLowerCase()) {
      case 'steam':
        return (
          <path d="M12 2a10 10 0 0 1 10 10 10 10 0 0 1-10 10C6.48 22 2 17.52 2 12L11.96 16v.02c0 1.09.89 1.98 1.98 1.98 1.09 0 1.98-.89 1.98-1.98v-.09l4.64-2.68c1.09 0 1.98-.89 1.98-1.98 0-1.09-.89-1.98-1.98-1.98-1.09 0-1.98.89-1.98 1.98v.06l-2.68 4.64c-.2.02-.41.04-.62.04-2.2 0-3.98-1.78-3.98-3.98 0-.34.04-.67.13-.98L2 12C2 6.48 6.48 2 12 2z" />
        )
      case 'bilibili':
        return (
      ...
      case 'netease':
        return (
          <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2zm-1 2v8h2V4h-2zm-4 4v4h2V8H7zm8 0v4h2V8h-2z" />
        )
      case 'github':
        return (
...
const ActivityItem = memo(
  ({
    ...
    const { title, details } = useMemo(
      () => getDetailedDescription(activity, t),
      [activity, t],
    )
      ...
      globalFetchPromise = (async () => {
        try {
          ...
          const apiEndpoint = API_URL
            ? `${API_URL}/api/activities?limit=8`
