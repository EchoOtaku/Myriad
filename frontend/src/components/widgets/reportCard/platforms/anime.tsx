 ...
 * Shared: library poster flip, rating badge, overview stats.
 */
import {
...
export const BangumiWidget = memo(({ data, showOverview, onContentChange }: any) => {
  const { t } = useI18n()
...
export const MalWidget = memo(({ data, showOverview, onContentChange }: any) => {
  const { t } = useI18n()
  const libraryItems = useMemo(
    () => data?.library_items || [],
    [data?.library_items],
