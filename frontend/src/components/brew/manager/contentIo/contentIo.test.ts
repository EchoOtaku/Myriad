...
describe('content Io text', () => {
  ...
  it('fileSlug 留下中文', () => {
    assert.equal(fileSlug('第一篇 手记', 0), '第一篇-手记')
...
<channel>
  ...
  it('往返保住标题、主题和状态', () => {
    const xml = serializeWordpressXml([sample], 'Brew')
...
<contents>
  ...
  <item>
    ...
    const wxr = serializeWordpressXml([sample], 'Brew')
...
describe('parseTransferFile', () => {
  it('按种类读 xml / json / md', async () => {
    const wp = await parseTransferFile(
      'wordpress',
      new File([serializeWordpressXml([sample], 'Brew')], 'a.xml'),
    )
