...
function fromDocs(docs: readonly BrewNoteDoc[]): TransferNote[] {
  return docs.map((doc) => ({
    title: doc.title,
...
export async function importTransferFile(
  ...
  try {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      try {
        const doc = await brewApi.createNoteDoc({
          title,
        ...
        if (note.status === 'published') {
          try {
            await brewApi.publishNoteDoc(doc.id, {
              title,
              content_md: note.content_md,
              topic: note.topic ?? null,
              published_at: note.published_at,
