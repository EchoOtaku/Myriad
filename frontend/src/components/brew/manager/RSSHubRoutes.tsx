...
export function RSSHubRouteExplorer({
  ...
  const { t } = useI18n()
  const brew = t.brew
  const names = brew.rsshubRouteNames as Record<string, string>
  ...
  const toItem = useCallback(
      ...
      return {
        ...
        badge:
          route.requiresConfig === 'server'
            ? { label: brew.rsshubServerConfig, tone: 'warn' }
            : route.requiresConfig === 'optional'
              ? { label: brew.rsshubOptionalConfig, tone: 'default' }
              : undefined,
        renderHit: ({
          leading,
          main,
    ...
    [
      brew.rsshubOptionalConfig,
      brew.rsshubServerConfig,
      disabled,
      handlePick,
      names,
      routePath,
  ...
  return (
    <div
      className={`brew-rsshub-routes${open ? ' is-open' : ''}${disabled ? ' is-disabled' : ''}`}
    >
      <button
        type="button"
        className="brew-rsshub-routes__summary"
        disabled={disabled}
        aria-expanded={open}
        onClick={() => {
      ...
      >
        <span className="brew-rsshub-routes__summary-main">
          <span className="brew-rsshub-routes__summary-name">
            {brew.rsshubBrowsePopular}
          </span>
          {currentLabel ? (
            <span className="brew-rsshub-routes__summary-url">
              {currentLabel}
            </span>
          ) : null}
        </span>
        <span className="brew-rsshub-routes__summary-side">
          <SettingTitleTag variant="muted">{ROUTE_COUNT}</SettingTitleTag>
          <ChevronDown className="brew-rsshub-routes__chevron" />
        </span>
      </button>

      {open ? (
        <div className="brew-rsshub-routes__panel">
          <div className="brew-rsshub-routes__cats">
            <SettingTitleTag
          ...
          <div className="brew-rsshub-routes__legend">
            <span className="brew-rsshub-routes__key brew-rsshub-routes__key--server">
              <Settings />
              {brew.rsshubServerConfig}
            </span>
            <span className="brew-rsshub-routes__key brew-rsshub-routes__key--optional">
              <Wrench />
              {brew.rsshubOptionalConfig}
            </span>
          </div>
          <ManagedList
            className="brew-rsshub-routes__list"
            items={routes.map(toItem)}
            emptyText={brew.rsshubBrowsePopular}
            maxHeight="11rem"
          />
          <SettingsButton
            size="sm"
