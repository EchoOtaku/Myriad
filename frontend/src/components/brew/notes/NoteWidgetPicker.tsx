...
export function NoteWidgetPicker({
  ...
  return createPortal(
    <div className="brew-note__widget-picker-layer" onMouseDown={onClose}>
      <div
        className="brew-skin brew-note__widget-picker"
        role="dialog"
        aria-label={t.brew.noteWidgetPicker}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="brew-note__widget-picker-head">
          <div className="brew-note__widget-picker-search">
            <FaSearch size={12} aria-hidden />
            <input
              type="search"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder={t.brew.noteWidgetSearch}
              aria-label={t.brew.noteWidgetSearch}
              autoComplete="off"
            />
          </div>
          <button
            type="button"
            className="brew-note__widget-picker-close"
            onClick={onClose}
        ...
        <div className="brew-note__widget-picker-list">
          {filtered.length === 0 ? (
            <p className="brew-note__widget-picker-empty">{t.brew.noteWidgetEmpty}</p>
          ) : (
            filtered.map((widget) => {
              ...
              return (
                <button
                  key={widget.id}
                  type="button"
                  className="brew-note__widget-picker-item"
                  onClick={() => onPick(widget.id, size)}
                >
                  <span className="brew-note__widget-picker-name">{label}</span>
                  <span className="brew-note__widget-picker-size">{size}</span>
                </button>
              )
            })
          )}
