...
describe('article prefetch cancel', () => {
  it('hover leave cancels the in-flight prefetch', () => {
    ...
    const filter = readFileSync(join(brewDir, 'BrewFilterLane.tsx'), 'utf8')
    const list = readFileSync(join(brewDir, 'skin/BrewList.tsx'), 'utf8')
    ...
    assert.match(list, /onPeekEnd=\{editMode \? undefined : onPeekEnd\}/)
