...
describe('BrewPeekAir', () => {
  it('封面 peek 换全局背景，叠标题和站点，进出有过渡和渐变', () => {
    ...
    assert.match(page, /<BrewFilterLane[\s\S]*onPeekItem=\{handlePeekItem\}/)
    assert.match(page, /<BrewFilterLane[\s\S]*onPeekEnd=\{handlePeekEnd\}/)
    const filter = readFileSync(join(dir, '../BrewFilterLane.tsx'), 'utf8')
    const list = readFileSync(join(dir, '../skin/BrewList.tsx'), 'utf8')
    assert.match(filter, /onPeekItem=\{starredMode\?\.isEditMode \? undefined : onPeekItem\}/)
    assert.match(list, /onPeek=\{/)
    ...
    assert.match(air, /BREW_PEEK_HANDOFF_MS/)
    assert.match(tokens, /scale\(1\.03\)/)
    assert.match(tokens, /\.brew-peek-air\.is-swap/)
    assert.match(page, /if \(next\) setPeekFace/)
    assert.match(feeds, /event\.currentTarget\.contains\(to\)/)
    ...
    assert.match(storyCard, /data-brew-peek-lane/)
