        ...
        sources.reduce(
          (acc, source) => {
            if (source.category) {
              ...
              cats.forEach((cat) => {
                if (!acc[cat]) acc[cat] = []
                if (!acc[cat].some((s) => s.id === source.id)) {
                  acc[cat].push(source)
                }
              })
            } else {
          ...
          {stats && (
            <div className="px-4 py-3 border-b border-white/10 text-sm text-gray-400">
              <div className="flex items-center justify-between">
                <span>
                  {t.brew.sourcesCount.replace(
                    '{count}',
                    String(stats.total_sources),
                  )}
                </span>
                <span className="text-blue-400">
                  {t.brew.unreadCount.replace(
                    '{count}',
                    String(stats.total_unread),
                  )}
                </span>
              </div>
            </div>
            ...
            {Object.entries(sourcesByCategory).map(([category, catSources]) => (
              <div key={category} className="mb-2">
                ...
                <button
                  onClick={() =>
                    onCategorySelect(
                      category === t.brew.uncategorized ? null : category,
                    )
                  }
