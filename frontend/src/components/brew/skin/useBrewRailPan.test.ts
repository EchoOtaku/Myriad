...
describe('railCardKeepsPaint / railMountColumns', () => {
  ...
  it('短栅只排实装列，总宽仍按全列，视觉列不改绝对列', () => {
    ...
    assert.equal(
      start.template,
      'repeat(12, var(--brew-story-w)) minmax(0, calc(28 * (var(--brew-story-w) + 0.75rem) - 0.75rem))',
    )
    ...
    assert.equal(storyMountGrid(1, 40, 40).template, 'repeat(40, var(--brew-story-w))')
    ...
    assert.equal(storyRailTrackSize(1), 'var(--brew-story-w)')
    assert.equal(
      storyRailTrackSize(40),
      'calc(40 * (var(--brew-story-w) + 0.75rem) - 0.75rem)',
    )
  })
  ...
  it('只预热视口和右侧预显的封面，扩窗空列不解码', () => {
        ...
        children: [
          {
            dataset: { railCol: '5' },
            classList: {
              contains: (name: string) => name === 'brew-story--slot',
              remove() {},
            },
    ...
    assert.match(slot.className, /brew-story__hit/)
...
describe('useBrewRailPan 热路', () => {
  it('保绘轨不每帧排退场，跟轨不扫领头卡', () => {
    const src = readFileSync(
      join(dirname(fileURLToPath(import.meta.url)), 'useBrewRailPan.ts'),
      'utf8',
    )
    ...
    assert.doesNotMatch(seek, /brewRailScroll/)
...
describe('storyCardFace', () => {
  it('同一篇文章面对象沿用，字段变了才重算', async () => {
    ...
    assert.match(inner, /brew-story__title/)
