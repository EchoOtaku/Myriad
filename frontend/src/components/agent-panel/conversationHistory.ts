...
export function useConversationHistory(ids: readonly string[]) {
  ...
  const start =
    window.identity !== identity || (follow && window.count !== ids.length)
      ? historyStart(ids.length)
      : window.start
  if (window.identity !== identity || window.count !== ids.length) {
    setWindow({ identity, start, count: ids.length })
  }
  useLayoutEffect(() => {
    // Let the pan observer restore the retained row before accepting another
    // near-start notification from the previous page's geometry.
    let frame = requestAnimationFrame(() => {
      frame = requestAnimationFrame(() => { pending.current = false })
    })
  ...
  const onNewer = useCallback(() => {
    setWindow({
      identity,
      start: Math.min(historyStart(ids.length), start + HISTORY_PAGE_SIZE),
      count: ids.length,
    })
  }, [identity, start, ids.length])
