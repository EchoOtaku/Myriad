...
it('toggles the same panel off and never leaves two panels open', () => {
  ...
  assert.equal(
    currentReaderPanel({ toc: false, phantasiai: true, podcast: true }),
    'annotations',
  )
...
it('dialog and popup triggers share expanded/controls/haspopup', () => {
  assert.deepEqual(readerDialogTrigger(true, 'phantasi-reader-tool-sheet'), {
    'aria-expanded': true,
    'aria-haspopup': 'dialog',
    'aria-controls': 'phantasi-reader-tool-sheet',
  })
  assert.deepEqual(readerPopupTrigger(false, 'phantasi-reader-toc-panel'), {
    'aria-expanded': false,
    'aria-haspopup': true,
    'aria-controls': 'phantasi-reader-toc-panel',
  })
})
