...
import { PodcastPlayer } from '../../../../services/phantasiaiApi'
import {
...
export function usePodcast({
        ...
        if (engine === 'cloud') {
          if (cloudTtsAvailable === null) {
            ...
            try {
              ...
              if (!status.available || !status.tts_enabled) {
                setCloudTtsAvailable(false)
                setCloudTtsError(status.error || t.phantasi.cloudTtsUnavailable)
                showToastMessage(
                  status.error || t.phantasi.cloudTtsUnavailableCheck,
                  5000,
                )
                setTtsEngine('system')
              ...
              const errMsg = userFacingError(err, t.phantasi.cannotConnectSpeech)
              setCloudTtsAvailable(false)
              setCloudTtsError(errMsg)
              showToastMessage(`${t.phantasi.cloudTtsUnavailable}: ${errMsg}`, 5000)
              setTtsEngine('system')
              saveTTSSettings({ engine: 'system' })
              return
            }
            ...
            showToastMessage(
              cloudTtsError || t.phantasi.cloudTtsUnavailableCheck,
              5000,
            )
            setTtsEngine('system')
            saveTTSSettings({ engine: 'system' })
          ...
          if (cloudPodcastPlayerRef.current?.hasAudio()) {
            ...
            try {
              ...
              if (completeCache) {
                ...
                showToastMessage(
                  `${t.phantasi.switchedToCloudTts}（${t.phantasi.cached}: ${completeCache.voiceName}）`,
                )
              } else {
                setTtsEngine('system')
                saveTTSSettings({ engine: 'system' })
              ...
              showToastMessage(t.phantasi.checkCacheFailed)
            }
          }
        } else {
          if (podcastPlayerRef.current) {
          ...
          showToastMessage(t.phantasi.switchedToSystemTts)
        }
      } else {
        showToastMessage(
          engine === 'cloud'
            ? t.phantasi.switchedToCloudTts
            : t.phantasi.switchedToSystemTts,
        )
      }
    },
    [
  ...
  const handleVoiceChange = useCallback(
    (role: 'host' | 'guest', voiceId: number) => {
      ...
      showToastMessage(t.phantasi.voiceSettingSaved)
    },
    [showToastMessage, t],
  )
  ...
  const handleSwitchToVoice = useCallback(
    async (voiceId: number, role: string) => {
      ...
      if (!voiceCache || voiceCache.file_count < podcastDialogues.length) {
        showToastMessage(`${voiceName} ${t.phantasi.reload}`)
        return
      }
        ...
        try {
          ...
          if (result) {
            if (result.cacheHits === result.total) {
              showToastMessage(`${voiceName}（${t.phantasi.cached}）`)
            } else {
              showToastMessage(
          ...
          showToastMessage(t.phantasi.switchFailed)
        } finally {
          if (isCurrent()) {
  ...
  const handleClearVoiceCache = useCallback(
    async (voiceId: number) => {
      ...
      try {
        ...
        showToastMessage(t.phantasi.clearFailed)
      } finally {
        if (isCurrent()) {
  ...
  const loadPodcast = useCallback(async () => {
    ...
    if (!isPhantasiai || podcastLoading || cloudTtsLoading) return
    ...
    try {
      const response = await phantasiaiApi.getPodcastScript(itemId, signal)
      ...
      setPodcastError(userFacingError(err, t.phantasi.generatePodcastFailed))
    } finally {
  ...
  const regeneratePodcast = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showToastMessage(t.phantasi.switchedToSystemTts, 3000)
      } else {
        setPodcastError(response.error || t.phantasi.regenerateFailed)
        showToastMessage(response.error || t.phantasi.regenerateFailed, 3000)
      }
    } catch (err) {
      if (!isCurrent() || signal.aborted) return

      console.error('Failed to regenerate podcast:', err)
      const errMsg = userFacingError(err, t.phantasi.generatePodcastFailed)
  ...
  }, [isPhantasiai, podcastLoading, cloudTtsLoading, itemId, showToastMessage, t])
    ...
    try {
      ...
      if (result) {
        if (result.generated > 0) {
          ...
          showToastMessage(t.phantasi.cached)
        }
      }
    } catch (err) {
      if (!isCurrent()) return

      const errMsg = userFacingError(err, t.phantasi.loadFailed)
      console.error('Failed to reload cloud TTS:', errMsg, err)
      showToastMessage(`${t.phantasi.cloudTtsUnavailable}: ${errMsg}`, 3000)
    } finally {
      if (isCurrent()) {
        setCloudTtsLoading(false)
      }
