    ...
    const handleContentClick = async (e: Event) => {
      ...
      if (target.tagName === 'IMG') {
        ...
        const isInEmbedCard = img.closest(
          '.note-widget, .phantasi-embed-card, .phantasi-embed-exempt, .phantasi-bilibili-embed, .phantasi-netease-music, .phantasi-steam-game, .phantasi-bilibili-video',
        )
      ...
      const neteaseCard = target.closest('.phantasi-netease-music')
      if (neteaseCard) {
        e.preventDefault()
        e.stopPropagation()
