...
export function ReaderTopicField({
  ...
  const label = topic
    ? topicDisplayName({ key: topic }, t.phantasi)
    : t.phantasi.topicNone
  ...
  return (
    <div className="phantasi-reader-topic" ref={rootRef}>
      {canEdit ? (
        <button
          type="button"
          className="phantasi-reader-topic__chip"
          aria-expanded={open}
        ...
        <span className="phantasi-reader-topic__chip is-static">
          <LuTag />
          <span>{label}</span>
        </span>
      )}
      {canEdit && open ? (
        <div className="phantasi-reader-topic__pop" id={panelId} role="dialog">
          <input
            className="phantasi-reader-topic__input"
            value={draft}
            disabled={busy}
            placeholder={t.phantasi.topicInputPlaceholder}
            aria-label={t.phantasi.topicNone}
          ...
          />
          {options.length > 0 ? (
            <div className="phantasi-reader-topic__choices">
              {options.map((name) => (
                <button
                  key={name}
                  type="button"
                  className={
                    name === topic
                      ? 'phantasi-reader-topic__choice is-current'
                      : 'phantasi-reader-topic__choice'
                  }
                  disabled={busy}
                  onClick={() => void commit(name)}
                >
                  {topicDisplayName({ key: name }, t.phantasi)}
                </button>
              ))}
            </div>
          ) : null}
          <div className="phantasi-reader-topic__actions">
            <button
              type="button"
              className="phantasi-reader-topic__action"
              disabled={busy}
              onClick={() => void recommend()}
            >
              <LuSparkles />
              {t.phantasi.topicRecommend}
            </button>
            {topic ? (
              <button
                type="button"
