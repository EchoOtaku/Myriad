...
it('notes expose early pages, retain them on failure, and retry the failed page', async () => {
  ...
  globalThis.fetch = (async () => {
    ...
    return Response.json({ items: [{ id: calls === 1 ? 2 : 1, title: 'note', source_id: 55, published_at: calls === 1 ? 2 : 1 }], next_cursor: calls === 1 ? 'next' : null })
  }) as typeof fetch
  function Harness() {
  ...
  try {
    await act(async () => { root.render(createElement(Harness)); await new Promise(resolve => setTimeout(resolve, 10)) })
    assert.deepEqual(state.notes.map(note => note.id), [2])
    assert.equal(state.loading, true)
    await act(async () => { second.reject(new Error('offline')) })
    assert.equal(state.failed, true)
    assert.equal(state.loading, false)
    assert.deepEqual(state.notes.map(note => note.id), [2])
    await act(async () => { state.retry(); await new Promise(resolve => setTimeout(resolve, 10)) })
    // Allow the cached first page to yield before consuming the next one.
    await act(async () => { await new Promise(resolve => setTimeout(resolve, 10)) })
    assert.deepEqual(state.notes.map(note => note.id), [2, 1])
