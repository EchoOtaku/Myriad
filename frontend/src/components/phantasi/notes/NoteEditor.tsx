...
import { phantasiAnimationPresets, getPhantasiTransition, usePhantasiAnimationConfig } from '../../../hooks/animation/pages/phantasi'
...
export default function NoteEditor({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setSaved(next)
        if (doc.last_error) {
          showNoteNotice(userFacingError(doc.last_error, t.phantasi.noteScheduleFailed))
        }
      } catch (err) {
        if (!controller.signal.aborted) {
          showNoteNotice(userFacingError(err, t.phantasi.errorLoadFailed))
        }
      } finally {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      phantasiApi.listNoteDocs(controller.signal).catch(() => []),
      phantasiApi
        .getItemPreviews(
  ...
  }, [contentMd, loading, pane, t.phantasi.notePreviewFailed])
  ...
  const fieldMessage = useCallback(
    (kind: ReturnType<typeof noteFieldError>) => {
      if (kind === 'empty-title') return t.phantasi.noteTitleRequired
      if (kind === 'title-too-long') {
        return format(t.phantasi.noteTitleTooLong, {
          max: MAX_NOTE_TITLE_CHARS,
          chars: countNoteChars(title.trim()),
        })
      }
      if (kind === 'body-too-long') {
        return format(t.phantasi.noteBodyTooLong, {
    ...
    [contentMd, format, t.phantasi.noteBodyTooLong, t.phantasi.noteTitleRequired, t.phantasi.noteTitleTooLong, title],
  )
  ...
  const handleSchedule = useCallback(async () => {
    ...
    if (timeError === 'missing-time') {
      showNoteNotice(t.phantasi.noteScheduleNeedTime)
      return
    }
    if (timeError === 'already-due') {
      showNoteNotice(t.phantasi.noteSchedulePast)
      return
    }
    ...
    fieldMessage,
    t.phantasi.errorSaveFailed,
    t.phantasi.noteScheduleNeedTime,
    t.phantasi.noteSchedulePast,
  ])
  ...
  }, [cloudId, saving, t.phantasi.errorSaveFailed])

  const handleDelete = useCallback(async () => {
    if (saving) return
    if (!window.confirm(t.phantasi.noteDeleteConfirm)) return
    setSaving(true)
    try {
      if (noteId !== undefined) {
      ...
      showNoteNotice(userFacingError(err, t.phantasi.errorDeleteFailed))
      setSaving(false)
    }
  }, [
    noteId,
    ...
    onDeleted,
    onClose,
    t.phantasi.noteDeleteConfirm,
    t.phantasi.errorDeleteFailed,
  ])

  // Esc 关闭；有云端没存上的改动时先确认。确认丢弃就连本地草稿一起丢，别下次开门又冒出来。
  const requestClose = useCallback(() => {
    if (dirty) {
      if (!window.confirm(t.phantasi.noteDiscardConfirm)) return
      clearNoteDraft(draftKey)
      clearNoteDraft('new')
    }
    if (
      ...
      void phantasiApi.deleteNoteDoc(cloudId)
      clearNoteDraft(draftKey)
      clearNoteDraft('new')
    }
    onClose()
      ...
      <div className="phantasi-note__frame">
        ...
        <NoteSettingsDrawer
          ...
          onCreateTopic={(name) => {
            ...
            void phantasiApi.createCategory({ name: next }).catch(() => {})
          }}
          publishedAt={publishedAt}
