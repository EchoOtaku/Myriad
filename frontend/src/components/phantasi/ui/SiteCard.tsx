...
function SiteStack({
  ...
  useEffect(() => {
    if (!live || phantasiMotionQuiet() || poolRef.current.length < 2) return
    let dwell = 0
    let tuck = 0
    let move = 0
    const play = () => {
  ...
  return (
    <span className="phantasi-site__stack" aria-hidden>
      <span className="phantasi-site__stack-deck">
        {seats.map((seat) => (
          <span
            key={seat.id}
            className={cx('phantasi-site__stack-face', `is-${seat.slot}`)}
            style={
              seat.face.ink
...
function SiteCardInner({
  ...
  return (
    ...
    >
      ...
      {cover ? (
        <span className="phantasi-site__scene" aria-hidden>
          <img key={cover} src={cover} alt="" decoding="async" />
        </span>
      ) : icon ? (
        <span className="phantasi-site__bleed" aria-hidden>
          <img
            key={icon}
            src={icon}
            alt=""
      ...
      {editing ? <PhantasiPick on={!!picked} /> : null}
      {editing && onEdit ? (
        <button
          type="button"
          className="phantasi-site__edit"
          title={editLabel}
      ...
      {unread && unread > 0 ? (
        <span className="phantasi-site__unread">{unread > 99 ? '99+' : unread}</span>
      ) : null}
      <button
        type="button"
        className="phantasi-site__head"
        onClick={(event) => {
          event.stopPropagation()
          onActivate(id)
        }}
        aria-pressed={on}
      >
        <span className="phantasi-site__name">{name}</span>
        {!cover && description ? (
