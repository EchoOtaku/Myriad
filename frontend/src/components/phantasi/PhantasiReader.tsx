...
import type { PhantasiItem, SourceType } from '../../types/phantasi'
...
export default function PhantasiReader(props: PhantasiReaderProps) {
  ...
  const config = usePhantasiAnimationConfig()
...
function ReaderSessionBody({
  ...
  useImmersiveChrome('phantasi-reader', true)
  ...
  const animConfig = usePhantasiAnimationConfig()
  const readerTransition = useMemo(
    () => getPhantasiTransition(animConfig, 'reader'),
    [animConfig],
  )
  ...
  useEffect(() => {
    if (isPhantasiai && annotations.length === 0 && !annotationsLoading) {
      const timer = setTimeout(() => {
        loadAnnotations()
      }, 500)
      return () => clearTimeout(timer)
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      else if (next === 'annotations') setShowPhantasiaiPanel(false)
  ...
  const handleShare = async () => {
    try {
      ...
      showToastMessage(t.phantasi.linkCopied)
    } catch (err) {
  ...
  const readerAnimProps = useMemo(() => {
    ...
    return {
      initial: articleSwap ? false : phantasiAnimationPresets.readerEnter.initial,
  ...
  return (
    <motion.div
      ...
      className={`phantasi-skin phantasi-reader fixed inset-0 z-50 ${currentTheme.bg}`}
      data-phantasi-reader="true"
      data-phantasi-theme={theme}
    >
      ...
      >
        <div className="flex justify-center">
          <ReaderLeftPanel
            ...
            showPhantasiaiPanel={showPhantasiaiPanel}
            setShowPhantasiaiPanel={setShowPhantasiaiPanel}
      ...
      {syncPaused ? (
        <div className="phantasi-reader__conflict" role="status">
          <p>{t.phantasi.readingSyncConflict}</p>
          <div className="phantasi-reader__conflict-actions">
            <button type="button" className="phantasi-reader__btn" onClick={() => void recoverLocalProgress()}>
              {t.phantasi.readingSyncKeepLocal}
            </button>
            <button type="button" className="phantasi-reader__btn" onClick={() => void recoverRemoteProgress()}>
              {t.phantasi.readingSyncUseLatest}
            </button>
          </div>
        </div>
      ) : null}
