import type {
  AddSourceRequest,
  PhantasiCategory,
  PhantasiExportManifest,
  PipackCategory,
  PipackRsshubInstance,
  PipackSource,
  PhantasiSource,
  CardSize,
  RsshubInstance,
...
export const PIPACK_VERSION = '2.0'
export const PIPACK_LEGACY_VERSION = '1.0'

function isSupportedPipackVersion(version: string): boolean {
  return version === PIPACK_VERSION || version === PIPACK_LEGACY_VERSION
}

export function normalizePipackUrl(url: string): string {
...
export function categoryToPackEntry(category: PhantasiCategory): PipackCategory {
  return {
    name: category.name,
    icon: category.icon,
...
export function buildPipackManifest(input: {
  sources: PipackSource[]
  categories: PipackCategory[]
  rsshubInstances: PipackRsshubInstance[]
  exportedAt?: string
}): PhantasiExportManifest {
  return {
    version: PIPACK_VERSION,
...
function parsePackSource(raw: unknown): PipackSource | null {
  ...
  return {
    ...
    feed_type: (asString(entry.feed_type) as PipackSource['feed_type']) || 'rss',
...
function parsePackCategory(raw: unknown): PipackCategory | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const entry = raw as Record<string, unknown>
  const name = asString(entry.name)?.trim()
  if (!name) return null
...
function parsePackRsshubInstance(raw: unknown): PipackRsshubInstance | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const entry = raw as Record<string, unknown>
  const name = asString(entry.name)?.trim()
  const url = asString(entry.url)?.trim()
...
export function parsePipackManifest(data: unknown): PhantasiExportManifest {
  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    throw new Error('invalid_pipack')
  }
  const raw = data as Record<string, unknown>
  const version = asString(raw.version) ?? ''
  if (!isSupportedPipackVersion(version) || !Array.isArray(raw.sources)) {
    throw new Error('invalid_pipack')
  }
  const sources = raw.sources
    .map(parsePackSource)
    .filter((entry): entry is PipackSource => entry !== null)
  ...
  return {
    ...
    categories: Array.isArray(raw.categories)
      ? raw.categories
          ...
          .filter((entry): entry is PipackCategory => entry !== null)
      : [],
    rsshub_instances: Array.isArray(raw.rsshub_instances)
      ? raw.rsshub_instances
          .map(parsePackRsshubInstance)
          .filter((entry): entry is PipackRsshubInstance => entry !== null)
      : [],
  }
}
...
function addableSourceType(sourceType: SourceType): AddSourceRequest['source_type'] {
  if (sourceType === 'link' || sourceType === 'phantasiai' || sourceType === 'rss') {
    return sourceType
  }
  return 'rss'
}
