...
describe('addFieldKind / canAutoDiscover', () => {
  it('只有普通 RSS 才自动探测', () => {
    ...
    assert.equal(addFieldKind('phantasiai', 'rss'), 'rss')
    ...
    assert.equal(canAutoDiscover('phantasiai', 'rss'), true)
...
describe('resolveAddSourceType / canSubmitAdd', () => {
  it('RSSHub 开 AI 才落成 phantasiai；入口型必须有名字', () => {
    assert.equal(resolveAddSourceType('rsshub', true), 'phantasiai')
...
describe('pickAddKind / labels / opml', () => {
  it('选类型清 RSSHub 地址；文案键和扩展名对得上', () => {
    ...
    assert.equal(addSubmitLabelKey('phantasiai'), 'addPhantasiai')
