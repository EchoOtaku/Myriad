...
describe('pipack manifest', () => {
  it('round-trips portable source fields through v2 manifest', () => {
    ...
    assert.equal(manifest.version, PIPACK_VERSION)
    const parsed = parsePipackManifest(manifest)
    ...
    assert.equal(
      normalizePipackUrl(parsed.rsshub_instances?.[0].url ?? ''),
  ...
  it('accepts v1 packs and fills missing fields', () => {
    const parsed = parsePipackManifest({
      version: PIPACK_LEGACY_VERSION,
