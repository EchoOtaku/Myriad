...
describe('phantasiScore', () => {
  it('游客侧未读不影响分数', () => {
    const base = makeSource({ id: 1, unread_count: 0 })
    const loaded = makeSource({ id: 1, unread_count: 47 })
    assert.equal(phantasiScore(base, 'guest', NOW), phantasiScore(loaded, 'guest', NOW))
    assert.ok(phantasiScore(loaded, 'member', NOW) > phantasiScore(base, 'member', NOW))
  })

  it('失败源：游客沉底、管理员抬头', () => {
    const ok = makeSource({ id: 1, error_count: 0 })
    const broken = makeSource({ id: 2, error_count: 3 })
    assert.ok(phantasiScore(broken, 'guest', NOW) < phantasiScore(ok, 'guest', NOW))
    assert.ok(phantasiScore(broken, 'admin', NOW) > phantasiScore(ok, 'admin', NOW))
  })
})
