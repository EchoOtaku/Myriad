...
describe('notesRssUrl', () => {
  it('拼公开笔记 RSS，和后端同一条路径', () => {
    assert.equal(NOTES_RSS_PATH, '/phantasi/notes.xml')
    assert.match(rustPaths, /NOTES_RSS_PATH: &str = "\/phantasi\/notes.xml"/)
    ...
    assert.equal(notesRssUrl('https://ex.com/'), 'https://ex.com/phantasi/notes.xml')
    assert.equal(notesRssUrl('https://ex.com'), 'https://ex.com/phantasi/notes.xml')
    assert.equal(notesRssUrl(''), '/phantasi/notes.xml')
  })
})
...
describe('sourceRssShareUrl', () => {
  it('只给笔记源公开 RSS', () => {
    assert.equal(
      ...
      'https://ex.com/phantasi/notes.xml',
    )
    assert.equal(
      sourceRssShareUrl(
