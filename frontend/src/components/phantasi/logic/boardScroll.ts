...
const RAIL_TRACK = '[data-phantasi-rail-track]'
...
function readRails(root: ParentNode | null | undefined): Record<string, number> {
  ...
  root.querySelectorAll<HTMLElement>(RAIL_TRACK).forEach((el) => {
    const key = railKey(el.dataset.phantasiRailTrack)
    const value = Number(el.dataset.phantasiRailScroll)
...
function writeRails(
  ...
  for (const [key, value] of Object.entries(rails)) {
    ...
    const el = root.querySelector<HTMLElement>(
      `[data-phantasi-rail-track="${key}"]`,
    )
    if (el) el.dataset.phantasiRailRestore = String(value)
  }
}
