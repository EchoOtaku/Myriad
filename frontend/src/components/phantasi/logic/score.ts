...
export function lastPublishAt(s: PhantasiSource): number | null {
...
export function compareByScore(
  ...
  const bucketDelta =
    bucketScore(phantasiScore(b, role, now)) - bucketScore(phantasiScore(a, role, now))
  // 半档阈值，避免浮点尾差。
