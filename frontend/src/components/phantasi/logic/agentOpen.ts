...
interface AgentOpenHint {
  articleId?: string
  articleLink?: string
  openLatest?: boolean
}

export const PENDING_OPEN_KEY = 'phantasi_pending_open_article'
export const PENDING_READING_KEY = 'phantasi_pending_reading_list'
...
export async function findAgentArticle<T extends AgentArticle>(
  ...
  if (numericId != null) {
    try {
      ...
      console.warn('[Phantasi] Failed to fetch article by ID:', err)
    }
  }
  ...
  console.warn('[Phantasi] Article not found in API response. Looking for:', hint)
  return null
}
