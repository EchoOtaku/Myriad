...
it('captures rail pan offsets and writes restore tokens', () => {
  const root = {
    nodes: [
      { dataset: { phantasiRailTrack: 'sites', phantasiRailScroll: '80' } },
      { dataset: { phantasiRailTrack: 'items', phantasiRailScroll: '24' } },
    ],
    querySelectorAll() {
      return this.nodes
    },
    querySelector(selector: string) {
      const key = selector.match(/data-phantasi-rail-track="([a-z]+)"/)?.[1]
      return this.nodes.find((node) => node.dataset.phantasiRailTrack === key) ?? null
    },
  }
  const view = {
    scrollX: 0,
  ...
  assert.equal(root.nodes[0]?.dataset.phantasiRailRestore, '80')
  assert.equal(root.nodes[1]?.dataset.phantasiRailRestore, '24')
})
