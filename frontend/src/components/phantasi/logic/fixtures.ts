import type { PhantasiItem, PhantasiItemPreview, PhantasiSource } from '../../../types/phantasi'
...
export function makePreview(
  over: Partial<PhantasiItemPreview> = {},
): PhantasiItemPreview {
  previewSeq += 1
  return {
    id: previewSeq,
...
export function makeSource(over: Partial<PhantasiSource> = {}): PhantasiSource {
  return {
    id: 1,
    user_id: 1,
    name: '示例源',
...
export function makeItem(over: Partial<PhantasiItem> = {}): PhantasiItem {
  itemSeq += 1
  return {
    id: 1000 + itemSeq,
    source_id: 1,
