...
export function storyWindow(count: number, scroll: number, viewW: number, colW: number) {
  const cols = Math.ceil(count / 2)
  if (colW <= 1) return { from: 0, to: Math.min(count, BOOT_COLUMNS * 2) }
  const lead = Math.min(Math.max(0, cols - 1), Math.max(0, Math.floor(scroll / colW)))
  return {
    from: Math.max(0, lead - PAD_COLUMNS) * 2,
    to: Math.min(count, (lead + Math.max(1, Math.ceil(viewW / colW)) + PAD_COLUMNS) * 2),
  }
}
...
export function useStoryWindow(
  ...
  const update = useCallback((state: { scroll: number; viewW: number; colW: number }) => {
    const next = storyWindow(count, state.scroll, state.viewW, state.colW)
    setWindow(prev => prev.from === next.from && prev.to === next.to ? prev : next)
  }, [count])
  usePhantasiRailPan(
    viewportRef, trackRef, count > 0, originKey, '.phantasi-story',
    undefined, apiRef, true, update, onGrab,
  )
  useLayoutEffect(() => {
    ...
    if (scroll > max) { api.seek(max); api.refresh() }
    update({ scroll: Math.min(scroll, max), colW, viewW: viewportRef.current?.clientWidth ?? 0 })
  ...
  const onFocusCapture = useCallback((event: FocusEvent<HTMLDivElement>) => {
    const card = (event.target as HTMLElement).closest<HTMLElement>('[data-rail-col]')
  ...
  const onBlurCapture = useCallback((event: FocusEvent<HTMLDivElement>) => {
    if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setFocusColumn(null)
  }, [])
