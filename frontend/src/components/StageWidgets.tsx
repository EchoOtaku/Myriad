    ...
    useReportsVisibilityInterval(
      () =>
        setCurrentIndex((prev) =>
          libraryItems.length ? (prev + 1) % libraryItems.length : 0,
        ),
      !showOverview && libraryItems.length > 0 ? 5000 : null,
    )
      ...
      return (
        <div className="h-full w-full p-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-xs uppercase tracking-widest text-rose-500 font-bold mb-2">
              Bangumi
            </div>
            <div className="text-lg font-bold text-gray-900 dark:text-white line-clamp-3">
              {currentItem.title}
            </div>
            {currentItem.rate ? (
