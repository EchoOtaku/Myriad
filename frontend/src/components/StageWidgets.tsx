...
export const DanmakuWidget = memo(
  ({ data }: { data?: { danmaku?: string[] } }) => {
    const { t } = useI18n()
    const texts = useMemo(
      () => data?.danmaku || t.reportsPage.danmakuDefaults,
      [data?.danmaku, t.reportsPage.danmakuDefaults],
    )
...
// Steam统计展示
export const SteamStatsWidget = memo(
  ({
    data,
  }: {
    data?: {
    ...
    const type = useMemo(
      () => data?.player_type || t.reportsPage.casualPlayer,
      [data?.player_type, t.reportsPage.casualPlayer],
    )
...
// GitHub统计展示
export const GithubStatsWidget = memo(
  ({
    data,
  }: {
    data?: {
    ...
    const { t } = useI18n()
    const level = useMemo(
      () => data?.contribution_level || t.reportsPage.activeDeveloper,
      [data?.contribution_level, t.reportsPage.activeDeveloper],
    )
    ...
    const langSegments = useMemo(() => {
      const items = langs
        ...
        .slice(0, 4)
      const total = items.reduce((sum, l) => sum + l.percentage, 0)
      if (total === 0) return []
...
export const BangumiWidget = memo(
  ({
    ...
    // 类型占比（书/番/游戏/音乐/剧）与常用标签，此前后端已下发但未展示
    ...
    const barSegments = useMemo(() => {
      ...
      return typeDist.map(([type, count]) => {
        const segment = {
          type,
          count,
          pct: (count / totalSubjects) * 100,
          delay: baseDelay + (acc / totalSubjects) * fillDuration,
          duration: (count / totalSubjects) * fillDuration,
        }
        acc += count
        return segment
      })
    ...
    const wallCovers = useMemo(
      () => libraryItems.filter((item) => item.cover).slice(0, 5),
      [libraryItems],
    )
    ...
    useReportsVisibilityInterval(
      () =>
        setCurrentIndex((prev) =>
          libraryItems.length ? (prev + 2) % libraryItems.length : 0,
        ),
      !showOverview && libraryItems.length > 0 ? 5000 : null,
    )
