...
const MusicCard = React.memo<MusicCardProps>(({
  ...
  platformColor,
  isVip,
  isCurrentSong: _isCurrentSong,
  isPlaying,
  musicColor,
  animationDelay,
  onPlay,
}) => {
  // 提取艺术家信息 - useMemo 避免每次渲染重算
  ...
  }, [item.metadata.ar, item.metadata.artist])
    ...
    >
      ...
      <div className="relative bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-[1.02] overflow-hidden h-full">
        ...
        >
          ...
          {/* VIP 标识 */}
          {isVip && (
            <div className="absolute top-2 left-2 bg-linear-to-r from-yellow-400 to-orange-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg z-10">
              VIP
            </div>
          )}
          ...
          {/* 歌曲信息 */}
          <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 via-black/50 to-transparent p-3 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
