...
function Editor() {
  ...
  return <><button onClick={() => { setSurface('outline', 'fixture-token'); setGlowMode('none', 'fixture-token') }}>Save widget theme</button><button onClick={() => authSubject.change('next', true)}>Change subject</button><button onClick={() => setTitleFontSize(1.4, 'fixture-token')}>Save large</button><button onClick={() => { setTitleFontSize(0.8, 'fixture-token'); setTitleColor('accent', 'fixture-token') }}>Save size and color</button><button onClick={() => setTitleFontSize(1.4)}>Change size</button><button onClick={() => void setTitleFont('codystar', 'fixture-token')}>Font A</button><button onClick={() => void setTitleFont('henny-penny', 'fixture-token')}>Font B</button><span data-font>{titleFont}</span><span data-color>{titleColor}</span><span data-loading>{String(isLoading)}</span></>
}
function Fixture() {
