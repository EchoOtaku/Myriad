...
fn full_original_survives_prompt_building_and_selection_uses_bytes() {
    let mut req = request();
    req["content_md"] = json!(format!("{}{}", req["content_md"].as_str().unwrap(), "原文".repeat(20_000)));
...
fn selection_must_match_the_original_and_utf8_boundaries() {
    for selection in [json!({"start":1,"end":8,"text":"x"}), json!({"start":15,"end":21,"text":"different"}), json!({"start":21,"end":15,"text":"重复"})] {
        let mut req = request(); req["selection"] = selection;
        assert!(build_prompt(&req).is_err());
    }
}
#[test]
fn incomplete_or_empty_responses_are_rejected() {
    for response in [r#"{"replacement":"cut off""#, r#"{"replacement":"","complete":true}"#, r#"{"replacement":"partial","complete":false}"#] {
...
fn code_math_images_links_footnotes_and_widgets_cannot_disappear() {
    ...
    for (from, to) in [("/a.png","/b.png"), ("$x^2$","$x^3$"), ("let x = 1;","let x = 2;"), ("https://example.com","https://other.com"), (":::widget quote 2x2", ""), ("[^1]: Footnote", "") ] {
        let response = json!({"replacement":source.replace(from,to), "complete":true});
        assert!(validate_result(&req, &response.to_string()).is_err(), "lost {from}");
    }
}
