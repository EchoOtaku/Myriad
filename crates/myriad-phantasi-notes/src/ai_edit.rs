...
fn selection(request: &Value) -> Result<Option<(usize, usize)>, &'static str> {
    ...
    let start = value["start"].as_u64().and_then(|v| usize::try_from(v).ok()).ok_or("note_ai_invalid_request")?;
    let end = value["end"].as_u64().and_then(|v| usize::try_from(v).ok()).ok_or("note_ai_invalid_request")?;
    if start >= end || source.get(start..end) != value["text"].as_str() {
...
pub fn build_prompt(request: &Value) -> Result<String, &'static str> {
    ...
    if source.trim().is_empty() { return Err("note_ai_empty"); }
    // This is the note's product limit, not a claim about a model's context window.
    if crate::validate_note(if title.trim().is_empty() { "Untitled" } else { title }, source).is_err()
        || instruction.chars().count() > 4000 || locale.len() > 32
        || request["topic"].as_str().is_some_and(|s| s.chars().count() > 200) {
        return Err("note_ai_too_long");
    }
    if !request["topic"].is_null() && !request["topic"].is_string() { return Err("note_ai_invalid_request"); }
    selection(request)?;
    // Whitelist context fields: never forward arbitrary request properties.
    Ok(json!({"title":title,"topic":request["topic"],"content_md":source,
        "instruction":instruction,"locale":locale,"selection":request["selection"]}).to_string())
}

fn protected_parts(source: &str) -> HashMap<String, usize> {
    let mut parts = HashMap::new();
    let mut add = |key: String| { *parts.entry(key).or_insert(0) += 1; };
    let mut code: Option<String> = None;
    for event in Parser::new_ext(source, crate::markdown_options()) {
        match event {
            Event::Start(Tag::CodeBlock(_)) => code = Some(String::new()),
            Event::End(TagEnd::CodeBlock) => { if let Some(value) = code.take() { add(format!("code:{value}")); } },
...
pub fn validate_result(request: &Value, response: &str) -> Result<String, &'static str> {
    ...
    let replacement = value["replacement"].as_str().filter(|s| !s.trim().is_empty()).ok_or("note_ai_incomplete")?;
    if value["complete"] != true { return Err("note_ai_incomplete"); }
    let source = text(request, "content_md")?;
    ...
    if protected_parts(source).iter().any(|(key, count)| after.get(key).copied().unwrap_or(0) < *count) {
